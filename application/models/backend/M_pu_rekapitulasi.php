...
class M_pu_rekapitulasi extends CI_Model
{
    ...
    private function _apply_date_filter_pelaporan()
    {
        ...
        $this->db->group_start();
        $this->db->group_start();
        $this->db->where('pu_reimbust.kode_prepayment IS NOT NULL', null, false);
        $this->db->where('pu_reimbust.tgl_pengajuan >=', $tgl_awal);
        $this->db->where('pu_reimbust.tgl_pengajuan <=', $tgl_akhir);
        $this->db->group_end();

        $this->db->or_group_start();
        $this->db->where('pu_reimbust.kode_prepayment IS NULL', null, false);
        $this->db->where('pu_prepayment.tgl_prepayment >=', $tgl_awal);
        $this->db->where('pu_prepayment.tgl_prepayment <=', $tgl_akhir);
        $this->db->group_end();
        $this->db->group_end();
    }
    ...
    private function _build_base_query()
    {
        ...
        if ($tab === 'pelaporan') {
            ...
            // Rekapitulasi hanya untuk prepayment yang sudah paid & approved
            ...
            $this->db->where('pu_reimbust.status', 'approved');
            $this->db->group_end();
            $this->db->group_end();
        ...
        $this->db->group_by('pu_reimbust.id');
    }
    ...
    function get_total_pengeluaran()
    {
        ...
        $this->db->join('pu_prepayment AS c', 'c.kode_prepayment = a.kode_prepayment', 'inner');
    ...
    function get_data_reimbust($tgl_awal, $tgl_akhir)
    {
        ...
        // Rekap hanya untuk paid & approved (pu_reimbust selalu wajib)
        $this->db->where('a.payment_status', 'paid');
        $this->db->where('a.status', 'approved');
