...
class M_qbg_rekapitulasi extends CI_Model
{
    ...
    private function _apply_date_filter_pelaporan()
    {
        ...
        $this->db->group_start();
        $this->db->group_start();
        $this->db->where('qbg_reimbust.kode_prepayment IS NOT NULL', null, false);
        $this->db->where('qbg_reimbust.tgl_pengajuan >=', $tgl_awal);
        $this->db->where('qbg_reimbust.tgl_pengajuan <=', $tgl_akhir);
        $this->db->group_end();

        $this->db->or_group_start();
        $this->db->where('qbg_reimbust.kode_prepayment IS NULL', null, false);
        $this->db->where('qbg_prepayment.tgl_prepayment >=', $tgl_awal);
        $this->db->where('qbg_prepayment.tgl_prepayment <=', $tgl_akhir);
        $this->db->group_end();
        $this->db->group_end();
    }
    ...
    private function _build_base_query()
    {
        ...
        if ($tab === 'pelaporan') {
            ...
            $this->db->select(
                'qbg_reimbust.id,
                 ...
                 COALESCE(SUM(qbg_reimbust_detail.jumlah), qbg_prepayment.total_nominal) AS total_pengeluaran'
            );
        ...
        $this->db->select(
            'qbg_reimbust.id,
             ...
             qbg_reimbust.kode_reimbust,
             qbg_reimbust.sifat_pelaporan,
             qbg_reimbust.kode_prepayment,
             SUM(qbg_reimbust_detail.jumlah) AS total_jumlah_detail,
             COALESCE(SUM(qbg_reimbust_detail.jumlah), 0) AS total_pengeluaran'
        );
        ...
        $this->db->group_by('qbg_reimbust.id');
    }
    ...
    function get_total_pengeluaran()
    {
        ...
        $this->db->join('qbg_prepayment AS c', 'c.kode_prepayment = a.kode_prepayment', 'right');
        $this->db->group_start();
        $this->db->where('a.payment_status', 'paid');
    ...
    function get_data_reimbust($tgl_awal, $tgl_akhir)
    {
        ...
        $this->db->join('qbg_prepayment AS c', 'a.kode_prepayment = c.kode_prepayment', 'left');
        $this->db->where('a.payment_status', 'paid');
        $this->db->or_where('c.payment_status', 'paid');
