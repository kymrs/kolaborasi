...
class M_notifikasi extends CI_Model
{
    function pending_notification()
    {
        ...
        $pu_prepayment = $this->db->table_exists('pu_prepayment') ? $this->db->select('id')->from('pu_prepayment')->group_start()->where('app_name', $app)->where('app_status', 'waiting')->group_end()->or_group_start()->where('app2_name', $app)->where('app_status', 'approved')->where('app2_status', 'waiting')->group_end()->get()->num_rows() : null;
        ...
        $pu_reimbust = $this->db->table_exists('pu_reimbust') ? $this->db->select('id')->from('pu_reimbust')->group_start()->where('app_name', $app)->where('app_status', 'waiting')->or_group_start()->where('app2_name', $app)->where('app_status', 'approved')->where('app2_status', 'waiting')->group_end()->group_end()->get()->num_rows() : null;
        ...
        // var_dump($this->db->select('app_status')->from('pu_reimbust')->get()->result());
        ...
        $pu_deklarasi = $this->db->table_exists('pu_deklarasi') ? $this->db->select('id')->from('pu_deklarasi')->group_start()->where('app_name', $app)->where('app_status', 'waiting')->or_group_start()->where('app2_name', $app)->where('app_status', 'approved')->where('app2_status', 'waiting')->group_end()->group_end()->get()->num_rows() : null;
        ...
        $data['notif_menu'] = [
            'pengenumroh' => $pu_prepayment + $pu_reimbust + $pu_deklarasi,
            'sebelaswarna' => $prepayment_sw + $reimbust_sw + $deklarasi_sw,
            ...
            'quba' => $prepayment_qbg + $reimbust_qbg + $deklarasi_qbg,
