...
class M_kps_rekapitulasi extends CI_Model
{
    var $id = 'id';
    ...
    function _get_datatables_query()
    {
        ...
        if (!empty($_POST['tab'])) {
            if ($_POST['tab'] == 'pelaporan') {
                ...
                // Urutan ini mengikuti kolom yang dikirim pada controller get_list():
                // [0]no, [1]kode_prepayment, [2]kode_reimbust, [3]name, [4]tujuan, [5]tgl_pengajuan, [6]pengeluaran
                $this->column_order = array(null, 'kps_prepayment.kode_prepayment', 'kps_reimbust.kode_reimbust', 'tbl_data_user.name', 'kps_prepayment.tujuan', 'tgl_pengajuan', 'pengeluaran');
                $this->column_search = array('kps_prepayment.kode_prepayment', 'kps_reimbust.kode_reimbust', 'kps_reimbust.tgl_pengajuan', 'kps_prepayment.tgl_prepayment', 'tbl_data_user.name', 'kps_prepayment.tujuan');
                ...
                $this->db->select('kps_reimbust.id, 
                               ...
                               COALESCE(SUM(kps_reimbust_detail.jumlah), kps_prepayment.total_nominal) AS pengeluaran');
                ...
                $this->db->where('kps_reimbust.status', 'approved');
                $this->db->group_end();
                $this->db->group_end();
                ...
                // Urutan ini mengikuti kolom yang dikirim pada controller get_list():
                // [0]no, [1]kode_prepayment, [2]kode_reimbust, [3]name, [4]tujuan, [5]tgl_pengajuan, [6]pengeluaran
                $this->column_order = array(null, 'kps_reimbust.kode_prepayment', 'kps_reimbust.kode_reimbust', 'tbl_data_user.name', 'kps_reimbust.tujuan', 'kps_reimbust.tgl_pengajuan', 'total_jumlah_detail');
