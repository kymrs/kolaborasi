...
class M_ctz_rekapitulasi extends CI_Model
{
    var $id = 'id';
    var $table = 'ctz_reimbust';
    var $table2 = 'ctz_reimbust_detail';
    var $table3 = 'ctz_prepayment';
        ...
        if (!empty($_POST['tab'])) {
            if ($_POST['tab'] == 'pelaporan') {
                ...
                if (!empty($_POST['awal']) && !empty($_POST['akhir'])) {
                    ...
                    $this->db->group_start();
                    $this->db->where('ctz_reimbust.tgl_pengajuan >=', $tgl_awal);
                    $this->db->where('ctz_reimbust.kode_prepayment IS NOT NULL');
                    $this->db->or_where('ctz_prepayment.tgl_prepayment >=', $tgl_awal);
                    $this->db->where('ctz_reimbust.kode_prepayment IS NULL');
                    $this->db->group_end();

                    $this->db->group_start();
                    $this->db->where('ctz_reimbust.tgl_pengajuan <=', $tgl_akhir);
                    $this->db->where('ctz_reimbust.kode_prepayment IS NOT NULL');
                    $this->db->or_where('ctz_prepayment.tgl_prepayment <=', $tgl_akhir);
                    $this->db->where('ctz_reimbust.kode_prepayment IS NULL');
                    $this->db->group_end();
                }
                ...
                $this->db->group_by('ctz_reimbust.id, ctz_reimbust.kode_reimbust, ctz_reimbust.tgl_pengajuan');
            }
        }
        ...
        if (isset($_POST['order'])) {
            ...
            if (isset($_POST['tab'])) {
                if ($_POST['tab'] == 'pelaporan') {
                    ...
                    $this->db->order_by('ctz_reimbust.tgl_pengajuan', 'DESC');
                }
            } else {
