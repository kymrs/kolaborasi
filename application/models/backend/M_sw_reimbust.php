...
class M_sw_reimbust extends CI_Model
{
    // Reimbust
    var $id = 'id';
    ...
    // Deklarasi
    var $table3 = 'sw_deklarasi'; //nama tabel dari database
        ...
        if (!empty($_POST['status'])) {
            ...
            if ($_POST['status'] == 'on-process') {
                if ($alias != "eko") {
                    $this->db
                        ->group_start()
                        ->where('sw_reimbust.id_user', $id_user_logged_in)
    ...
    {
        $this->db->where('is_active', 1);
        $this->db->select('sw_prepayment.*, tbl_data_user.name'); // Memilih kolom dari kedua tabel
        ...
        $this->db->where('sw_prepayment.status', 'approved');
    ...
    public function delete($id)
    {
        ...
        if ($detail) {
            foreach ($detail as $rd) {
                ...
                $file_path = FCPATH . './assets/backend/document/reimbust/kwitansi/kwitansi_sw/' . $old_image;
        ...
        if ($detail) {
            foreach ($detail as $deklarasi) {
                ...
                $this->db->update('sw_deklarasi', ['is_active' => 1], ['kode_deklarasi' => $deklarasi]);
            }
        }
        ...
        $this->db->update('sw_prepayment', ['is_active' => 1], ['kode_prepayment' => $prepayment]);
    ...
    {
        return $this->db->distinct()->select('no_rek')->where('id_user', $id)->from('sw_reimbust')->get();
    }
}
