...
class M_sam_reimbust extends CI_Model
{
    // Reimbust
    var $id = 'id';
    var $table = 'sam_reimbust'; //nama tabel dari database
    ...
    // Deklarasi
    var $table3 = 'sam_deklarasi'; //nama tabel dari database
    ...
    {
        $this->db->select('sam_reimbust.*, tbl_data_user.name'); // Memilih kolom dari kedua tabel
        $this->db->from($this->table);
        $this->db->join('tbl_data_user', 'tbl_data_user.id_user = sam_reimbust.id_user', 'left'); // JOIN dengan tabel tbl_user
            ...
            if ($_POST['search']['value']) // jika datatable mengirimkan pencarian dengan metode POST
            {
                ...
                if ($i === 0) // looping awal
                {
                    ...
                    if ($item == 'name') {
                        ...
                        $this->db->like('sam_reimbust.' . $item, $_POST['search']['value']);
                    }
                } else {
                    if ($item == 'name') {
                        $this->db->or_like('tbl_data_user.' . $item, $_POST['search']['value']);
                    } else {
                        $this->db->or_like('sam_reimbust.' . $item, $_POST['search']['value']);
                    }
                }
        ...
        if (!empty($_POST['status'])) {
            ...
            if ($_POST['status'] == 'on-process') {
                if ($alias != "eko") {
                    $this->db
                        ->group_start()
                        ->where('sam_reimbust.id_user', $id_user_logged_in)
                ...
                if ($alias != "eko") {
                    $this->db
                        ...
                        ->or_where('sam_reimbust.id_user =' . $id_user_logged_in . ' AND (app4_status = "revised" OR app_status = "revised" OR app2_status = "revised")');
                } else {
        ...
        if (!empty($_POST['tab'])) {
            if ($_POST['tab'] == 'personal') {
                $this->db->where('sam_reimbust.id_user', $this->session->userdata('id_user'));
            } elseif ($_POST['tab'] == 'employee') {
                if ($alias != "eko") {
                    $this->db->group_start()
                        ->where('sam_reimbust.app_name =', "(SELECT name FROM tbl_data_user WHERE id_user = " . $this->session->userdata('id_user') . ")", FALSE)
                        ->where('sam_reimbust.id_user !=', $this->session->userdata('id_user'))
                        ->or_where('sam_reimbust.app2_name =', "(SELECT name FROM tbl_data_user WHERE id_user = " . $this->session->userdata('id_user') . ") && sam_reimbust.app_status = 'approved'", FALSE)
                        ->where('sam_reimbust.id_user !=', $this->session->userdata('id_user'))
                        ->or_where('sam_reimbust.app4_name =', "(SELECT name FROM tbl_data_user WHERE id_user = " . $this->session->userdata('id_user') . ")", FALSE)
                        ->where('sam_reimbust.id_user !=', $this->session->userdata('id_user'))
                        ->group_end();
                }
            }
            ...
            if ($_POST['search']['value']) // jika datatable mengirimkan pencarian dengan metode POST
            {
                ...
                if ($i === 0) // looping awal
                {
                    ...
                    if ($item == 'name') {
                        ...
                        $this->db->like('sam_deklarasi.' . $item, $_POST['search']['value']);
                    }
                } else {
                    if ($item == 'name') {
                        $this->db->or_like('tbl_data_user.' . $item, $_POST['search']['value']);
                    } else {
                        $this->db->or_like('sam_deklarasi.' . $item, $_POST['search']['value']);
                    }
                }
    ...
    {
        $this->db->where('is_active', 1);
        $this->db->select('sam_prepayment.*, tbl_data_user.name'); // Memilih kolom dari kedua tabel
        $this->db->from($this->table4);
        $this->db->join('tbl_data_user', 'tbl_data_user.id_user = sam_prepayment.id_user', 'left'); // JOIN dengan tabel tbl_user
        $id_user_logged_in = $this->session->userdata('id_user'); // Mengambil id_user dari sesi pengguna yang login
        $this->db->where('sam_prepayment.id_user', $id_user_logged_in);
            ...
            if ($_POST['search']['value']) // jika datatable mengirimkan pencarian dengan metode POST
            {
                ...
                if ($i === 0) // looping awal
                {
                    ...
                    if ($item == 'name') {
                        ...
                        $this->db->like('sam_prepayment.' . $item, $_POST['search']['value']);
                    }
                } else {
                    if ($item == 'name') {
                        $this->db->or_like('tbl_data_user.' . $item, $_POST['search']['value']);
                    } else {
                        $this->db->or_like('sam_prepayment.' . $item, $_POST['search']['value']);
                    }
                }
    ...
    public function max_kode($date)
    {
        ...
        $where = 'id=(SELECT max(id) FROM sam_reimbust where SUBSTRING(kode_reimbust, 2, 4) = ' . $formatted_date . ')';
        $this->db->where($where);
        $query = $this->db->get('sam_reimbust');
        return $query;
    }
    ...
    {
        // Ambil data sam_reimbust_detail berdasarkan reimbust_id
        $detail = $this->db->get_where('sam_reimbust_detail', ['reimbust_id' => $id])->result_array();

        // untuk menghapus file gambar
        if ($detail) {
            foreach ($detail as $rd) {
                $old_image = $rd['kwitansi'];
                $file_path = FCPATH . './assets/backend/document/reimbust/kwitansi/kwitansi_sam/' . $old_image;
        ...
        //         $this->db->delete('sam_deklarasi');
        //     }
        // }

        // Ambil data sam_reimbust berdasarkan reimbust_id
        ...
        $this->db->update('sam_prepayment', ['is_active' => 1], ['kode_prepayment' => $prepayment]);
    ...
    {
        return $this->db->distinct()->select('no_rek')->where('id_user', $id)->from('sam_reimbust')->get();
    }
}
