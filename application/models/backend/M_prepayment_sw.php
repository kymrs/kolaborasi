...
class M_prepayment_sw extends CI_Model
{
    // INISIASI VARIABLE
    var $id = 'id';
    var $table = 'swa_prepayment';
    var $table2 = 'swa_prepayment_detail';
    ...
    function _get_datatables_query()
    {
        ...
        if (!empty($_POST['status'])) {
            ...
            if ($_POST['status'] == 'on-process') {
                ...
                $this->db->where('swa_prepayment.id_user =' . $id_user_logged_in . ' AND status = "on-process"')
                ...
                $this->db->where('app2_name = (SELECT name FROM tbl_data_user WHERE id_user = ' . $id_user_logged_in . ' AND app2_status = "revised")', NULL, FALSE)
                    ...
                    ->or_where('swa_prepayment.id_user =' . $id_user_logged_in . ' AND (app4_status = "revised" OR app_status = "revised" OR app2_status = "revised")');
        ...
        if (!empty($_POST['tab'])) {
            if ($_POST['tab'] == 'personal') {
                $this->db->where('swa_prepayment.id_user', $this->session->userdata('id_user'));
            } elseif ($_POST['tab'] == 'employee') {
                $this->db->group_start()
                    ->where('swa_prepayment.app_name =', "(SELECT name FROM tbl_data_user WHERE id_user = " . $this->session->userdata('id_user') . ")", FALSE)
                    ->where('swa_prepayment.id_user !=', $this->session->userdata('id_user'))
                    ->or_where('swa_prepayment.app2_name =', "(SELECT name FROM tbl_data_user WHERE id_user = " . $this->session->userdata('id_user') . ") && swa_prepayment.app_status = 'approved'", FALSE)
                    ->where('swa_prepayment.id_user !=', $this->session->userdata('id_user'))
                    ->or_where('swa_prepayment.app4_name =', "(SELECT name FROM tbl_data_user WHERE id_user = " . $this->session->userdata('id_user') . ")", FALSE)
                    ->where('swa_prepayment.id_user !=', $this->session->userdata('id_user'))
                    ->group_end();
            }
        }
        ...
        if (!empty($_POST['status'])) {
            ...
            if ($_POST['status'] == 'on-process') {
                ...
                $this->db->where('app_status', 'waiting')
                    ...
                    ->or_where('swa_prepayment.id_user =' . $id_user_logged_in . ' AND app_status = "approved" AND app2_status = "waiting"')
                    ->or_where('app2_name = (SELECT name FROM tbl_data_user WHERE id_user = ' . $id_user_logged_in . ' AND app_status = "approved" AND app2_status = "waiting" AND status != "rejected" AND status != "revised")', NULL, FALSE);
            } elseif ($_POST['status'] == 'approved') {
                // Conditions for 'approved' status
                ...
                $this->db->where('app2_name = (SELECT name FROM tbl_data_user WHERE id_user = ' . $id_user_logged_in . ' AND app2_status = "revised")', NULL, FALSE)
                    ->or_where('app_name = (SELECT name FROM tbl_data_user WHERE id_user = ' . $id_user_logged_in . ' AND app_status = "revised")', NULL, FALSE)
                    ->or_where('swa_prepayment.id_user =' . $id_user_logged_in . ' AND (app_status = "revised" OR app2_status = "revised")');
            } elseif ($_POST['status'] == 'rejected') {
                $this->db->where('status', $_POST['status']);
            }
    ...
    public function max_kode($date)
    {
        ...
        $where = 'id=(SELECT max(id) FROM swa_prepayment where SUBSTRING(kode_prepayment, 2, 4) = ' . $formatted_date . ')';
        $this->db->where($where);
        $query = $this->db->get('swa_prepayment');
        return $query;
    }
    ...
    {
        $query = $this->db->select('event')->from('swa_prepayment')->where('id', $id);
    ...
    {
        return $this->db->distinct()->select('no_rek')->from('swa_prepayment')->get();
    }
}
