...
class M_prepayment_by_moment extends CI_Model
{
    // INISIASI VARIABLE
    var $id = 'id';
    var $table = 'tbl_prepayment_by_moment';
    var $table2 = 'tbl_prepayment_detail_by_moment';
        ...
        if (!empty($_POST['status'])) {
            ...
            if ($_POST['status'] == 'on-process') {
                ...
                $this->db->where('app_status', 'waiting')
                    ...
                    ->or_where('tbl_prepayment_by_moment.id_user =' . $id_user_logged_in . ' AND app_status = "approved" AND app2_status = "waiting"')
                ...
                $this->db->where('app2_name = (SELECT name FROM tbl_data_user WHERE id_user = ' . $id_user_logged_in . ' AND app2_status = "revised")', NULL, FALSE)
                    ...
                    ->or_where('tbl_prepayment_by_moment.id_user =' . $id_user_logged_in . ' AND (app_status = "revised" OR app2_status = "revised")');
        ...
        if (!empty($_POST['tab'])) {
            if ($_POST['tab'] == 'personal') {
                ...
                $this->db->group_start()
                    ...
                    ->or_where('tbl_prepayment_by_moment.app2_name =', "(SELECT name FROM tbl_data_user WHERE id_user = " . $this->session->userdata('id_user') . ") && tbl_prepayment_by_moment.app_status = 'approved'", FALSE)
                    ->where('tbl_prepayment_by_moment.id_user !=', $this->session->userdata('id_user'))
                    ->group_end();
            }
        }
    ...
    public function max_kode($date)
    {
        ...
        $where = 'id=(SELECT max(id) FROM tbl_prepayment_by_moment where SUBSTRING(kode_prepayment, 2, 4) = ' . $formatted_date . ')';
        $this->db->where($where);
        $query = $this->db->get('tbl_prepayment_by_moment');
        return $query;
    }
