...
class M_reimbust_mc extends CI_Model
{
    // Reimbust
    var $id = 'id';
    var $table = 'mc_reimbust'; //nama tabel dari database
    ...
    // Deklarasi
    var $table3 = 'mc_deklarasi'; //nama tabel dari database
    ...
    {
        $this->db->select('mc_reimbust.*, tbl_data_user.name'); // Memilih kolom dari kedua tabel
        $this->db->from($this->table);
        $this->db->join('tbl_data_user', 'tbl_data_user.id_user = mc_reimbust.id_user', 'left'); // JOIN dengan tabel tbl_user
            ...
            if ($_POST['search']['value']) // jika datatable mengirimkan pencarian dengan metode POST
            {
                ...
                if ($i === 0) // looping awal
                {
                    ...
                    if ($item == 'name') {
                        ...
                        $this->db->like('mc_reimbust.' . $item, $_POST['search']['value']);
                    }
                } else {
                    if ($item == 'name') {
                        $this->db->or_like('tbl_data_user.' . $item, $_POST['search']['value']);
                    } else {
                        $this->db->or_like('mc_reimbust.' . $item, $_POST['search']['value']);
                    }
                }
        ...
        if (!empty($_POST['status'])) {
            ...
            if ($_POST['status'] == 'on-process') {
                ...
                $this->db->where('app_status', 'waiting')
                    ...
                    ->or_where('mc_reimbust.id_user =' . $id_user_logged_in . ' AND app_status = "approved" AND app2_status = "waiting"')
                ...
                $this->db->where('app2_name = (SELECT name FROM tbl_data_user WHERE id_user = ' . $id_user_logged_in . ' AND app2_status = "revised")', NULL, FALSE)
                    ...
                    ->or_where('mc_reimbust.id_user =' . $id_user_logged_in . ' AND (app_status = "revised" OR app2_status = "revised")');
        ...
        if (!empty($_POST['tab'])) {
            if ($_POST['tab'] == 'personal') {
                $this->db->where('mc_reimbust.id_user', $this->session->userdata('id_user'));
            } elseif ($_POST['tab'] == 'employee') {
                $this->db->group_start()
                    ->where('mc_reimbust.app_name =', "(SELECT name FROM tbl_data_user WHERE id_user = " . $this->session->userdata('id_user') . ")", FALSE)
                    ->where('mc_reimbust.id_user !=', $this->session->userdata('id_user'))
                    ->or_where('mc_reimbust.app2_name =', "(SELECT name FROM tbl_data_user WHERE id_user = " . $this->session->userdata('id_user') . ") && mc_reimbust.app_status = 'approved'", FALSE)
                    ->where('mc_reimbust.id_user !=', $this->session->userdata('id_user'))
                    ->group_end();
            }
        }
    ...
    private function _get_datatables_query2()
    {
        ...
        $this->db->select('mc_deklarasi.*, tbl_data_user.name');
        $this->db->from($this->table3);
        $this->db->join('tbl_data_user', 'tbl_data_user.id_user = mc_deklarasi.id_pengaju', 'left');
        $id_user_logged_in = $this->session->userdata('id_user'); // Mengambil id_user dari sesi pengguna yang login
            ...
            if ($_POST['search']['value']) // jika datatable mengirimkan pencarian dengan metode POST
            {
                ...
                if ($i === 0) // looping awal
                {
                    ...
                    if ($item == 'name') {
                        ...
                        $this->db->like('mc_deklarasi.' . $item, $_POST['search']['value']);
                    }
                } else {
                    if ($item == 'name') {
                        $this->db->or_like('tbl_data_user.' . $item, $_POST['search']['value']);
                    } else {
                        $this->db->or_like('mc_deklarasi.' . $item, $_POST['search']['value']);
                    }
                }
    ...
    function _get_datatables_query3()
    {
        ...
        $id_user_logged_in = $this->session->userdata('id_user'); // Mengambil id_user dari sesi pengguna yang login
        $this->db->where('mc_prepayment.id_user', $id_user_logged_in);
        $this->db->where('mc_prepayment.status', 'approved');
        ...
        if (!empty($_POST['tab'])) {
            if ($_POST['tab'] == 'personal') {
                ...
                $this->db->group_start()
                    ...
                    ->or_where('mc_prepayment.app2_name =', "(SELECT name FROM tbl_data_user WHERE id_user = " . $this->session->userdata('id_user') . ") && tbl_prepayment.app_status = 'approved'", FALSE)
                    ->where('mc_prepayment.id_user !=', $this->session->userdata('id_user'))
                    ->group_end();
            }
        }
    ...
    public function max_kode($date)
    {
        ...
        $where = 'id=(SELECT max(id) FROM mc_reimbust where SUBSTRING(kode_reimbust, 2, 4) = ' . $formatted_date . ')';
        $this->db->where($where);
        $query = $this->db->get('mc_reimbust');
        return $query;
    }
    ...
    public function delete($id)
    {
        ...
        $detail = $this->db->get_where('mc_reimbust_detail', ['reimbust_id' => $id])->result_array();

        // untuk menghapus file gambar
        if ($detail) {
            foreach ($detail as $rd) {
                $old_image = $rd['kwitansi'];
                $file_path = FCPATH . './assets/backend/document/reimbust/kwitansi_mc/' . $old_image;
        ...
        if ($detail) {
            foreach ($detail as $deklarasi) {
                ...
                $this->db->update('mc_deklarasi', ['is_active' => 1], ['kode_deklarasi' => $deklarasi]);
            }
        }

        // Ambil data tbl_reimbust berdasarkan reimbust_id
        $reimbust = $this->db->get_where('mc_reimbust', ['id' => $id])->row_array();
        ...
        $this->db->update('mc_prepayment', ['is_active' => 1], ['kode_prepayment' => $prepayment]);
    ...
    {
        return $this->db->distinct()->select('no_rek')->from('mc_reimbust')->get();
    }
}
