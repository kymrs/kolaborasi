...
class M_pu_mom_admin extends CI_Model
{
    var $id = 'id';
    var $table = 'pu_mom'; //nama tabel dari database
    ...
    public function max_kode()
    {
        ...
        $where = 'id=(SELECT max(id) FROM pu_mom where SUBSTRING(no_dok, 4, 4) = ' . date('ym') . ')';
        $this->db->where($where);
        $query = $this->db->get('pu_mom');
        return $query;
    }
