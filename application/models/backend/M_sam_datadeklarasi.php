...
class M_sam_datadeklarasi extends CI_Model
{
    var $id = 'id';
    var $table = 'sam_deklarasi'; //nama tabel dari database
    ...
    public function max_kode($date)
    {
        ...
        $where = 'id=(SELECT max(id) FROM sam_deklarasi where SUBSTRING(kode_deklarasi, 2, 4) = ' . $formatted_date . ')';
        $this->db->where($where);
        $query = $this->db->get('sam_deklarasi');
        return $query;
    }
