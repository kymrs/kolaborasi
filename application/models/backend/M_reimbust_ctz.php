...
class M_reimbust_ctz extends CI_Model
{
    // Reimbust
    var $id = 'id';
        ...
        if (!empty($_POST['tab'])) {
            if ($_POST['tab'] == 'personal') {
                ...
                $this->db->group_start()
                    ...
                    ->or_where('ctz_prepayment.app2_name =', "(SELECT name FROM tbl_data_user WHERE id_user = " . $this->session->userdata('id_user') . ") && tbl_prepayment.app_status = 'approved'", FALSE)
                    ->where('ctz_prepayment.id_user !=', $this->session->userdata('id_user'))
                    ->group_end();
            }
        }
