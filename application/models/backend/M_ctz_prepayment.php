...
class M_ctz_prepayment extends CI_Model
{
    // INISIASI VARIABLE
    var $id = 'id';
    var $table = 'ctz_prepayment';
    var $table2 = 'ctz_prepayment_detail';
    ...
    public function max_kode($date)
    {
        ...
        $where = 'id=(SELECT max(id) FROM ctz_prepayment where SUBSTRING(kode_prepayment, 2, 4) = ' . $formatted_date . ')';
        $this->db->where($where);
        $query = $this->db->get('ctz_prepayment');
        return $query;
    }
    ...
    {
        return $this->db->distinct()->select('no_rek')->where('id_user', $id)->from('ctz_prepayment')->get();
    }
}
