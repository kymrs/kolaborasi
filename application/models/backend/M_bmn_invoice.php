...
class M_bmn_invoice extends CI_Model
{
    // INISIASI VARIABLE
    var $id = 'id';
    ...
    {
        $this->db->where('id', $id);
        return $this->db->get('bmn_invoice')->row();
    }
    ...
    public function get_detail($id)
    {
        ...
        return $this->db->get('bmn_detail_invoice')->result();
    }
    ...
    {
        $this->db->where('invoice_id', $id);
        return $this->db->get('bmn_rek_invoice')->result();
    }
    ...
    public function max_kode($date)
    {
        ...
        $where = 'id=(SELECT max(id) FROM bmn_invoice where SUBSTRING(kode_invoice, 6, 4) = ' . $formatted_date . ')';
        $this->db->where($where);
        $this->db->from('bmn_invoice');
        $query = $this->db->get();
        return $query;
    ...
    // UNTUK QUERY INSERT DATA KE bmn_rek_invoice
    ...
    // UNTUK QUERY INSERT DATA KE bmn_detail_invoice
    public function save_detail2($data)
    {
        $this->db->insert_batch('bmn_detail_invoice', $data);
        return $this->db->insert_id();
    }
    ...
    {
        return $this->db->distinct()->select('nama, nama_bank, no_rek')->from('bmn_rek_invoice')->get();
    }
}
