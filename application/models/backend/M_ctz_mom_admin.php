...
class M_ctz_mom_admin extends CI_Model
{
    var $id = 'id';
    var $table = 'ctz_mom'; //nama tabel dari database
    ...
    public function max_kode()
    {
        ...
        $where = 'id=(SELECT max(id) FROM ctz_mom where SUBSTRING(no_dok, 4, 4) = ' . date('ym') . ')';
        $this->db->where($where);
        $query = $this->db->get('ctz_mom');
        return $query;
    }
