...
class M_notifikasi extends CI_Model
{
    function pending_notification()
    {
        ...
        $reimbust_pu = $this->db->table_exists('tbl_reimbust_pu') ? $this->db->select('id')->from('tbl_reimbust_pu')->where('app_name', $app)->where_not_in('app_status', ['approved', 'rejected', 'revised'])->or_where('app2_name', $app)->where('app_status', 'approved')->where('app2_status', 'waiting')->get()->num_rows() : null;
