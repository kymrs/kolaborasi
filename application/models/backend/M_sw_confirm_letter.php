...
class M_sw_confirm_letter extends CI_Model
{
    var $id = 'id';
    var $table = 'sw_confirm_letter'; //nama tabel dari database
    ...
    {
        $header = $this->db
            ->get_where('sw_confirm_letter', ['id' => $id])
            ->row();

        $details = $this->db
            ->get_where('sw_confirm_letter_detail', ['letter_id' => $id])
            ->result();
        ...
        foreach ($details as $row) {
            if ($row->item_type == 'package') {
                $package[] = $row;
            } elseif ($row->item_type == 'additional') {
                $additional[] = $row;
            }
        }

        $header->package = $package;
        $header->additional = $additional;
