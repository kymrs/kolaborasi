...
class M_pu_datanotifikasi extends CI_Model
{
    var $id = 'id';
    var $table = 'pu_notifikasi'; //nama tabel dari database
    ...
    private function _get_datatables_query()
    {
        ...
        $this->db->select('pu_notifikasi.*, tbl_data_user.name');
        $this->db->from('pu_notifikasi');
        $this->db->join('tbl_data_user', 'tbl_data_user.id_user = pu_notifikasi.id_user');
            ...
            if ($_POST['search']['value']) // jika datatable mengirimkan pencarian dengan metode POST
            {
                ...
                if ($i === 0) // looping awal
                {
                    ...
                    if ($item == 'name') {
                        ...
                        $this->db->like('pu_notifikasi.' . $item, $_POST['search']['value']);
                    }
                } else {
                    if ($item == 'name') {
                        $this->db->or_like('tbl_data_user.' . $item, $_POST['search']['value']);
                    } else {
                        $this->db->or_like('pu_notifikasi.' . $item, $_POST['search']['value']);
                    }
                }
        ...
        if (!empty($_POST['status'])) {
            ...
            if ($_POST['status'] == 'on-process') {
                ...
                $this->db->where('app_hc_status', 'waiting')
                    ...
                    ->or_where('pu_notifikasi.id_user =' . $id_user_logged_in . ' AND app_hc_status = "approved" AND app2_status = "waiting"')
                ...
                $this->db->where('app2_name = (SELECT name FROM tbl_data_user WHERE id_user = ' . $id_user_logged_in . ' AND app2_status = "revised")', NULL, FALSE)
                    ...
                    ->or_where('pu_notifikasi.id_user =' . $id_user_logged_in . ' AND (app_hc_status = "revised" OR app2_status = "revised")');
        ...
        if (!empty($_POST['tab'])) {
            if ($_POST['tab'] == 'personal') {
                $this->db->where('pu_notifikasi.id_user', $this->session->userdata('id_user'));
            } elseif ($_POST['tab'] == 'employee') {
                $this->db->group_start()
                    ->where('pu_notifikasi.app_hc_name =', "(SELECT name FROM tbl_data_user WHERE id_user = " . $this->session->userdata('id_user') . ")", FALSE)
                    ->where('pu_notifikasi.id_user !=', $this->session->userdata('id_user'))
                    ->or_where('pu_notifikasi.app2_name =', "(SELECT name FROM tbl_data_user WHERE id_user = " . $this->session->userdata('id_user') . ") && pu_notifikasi.app_hc_status = 'approved'", FALSE)
                    ->where('pu_notifikasi.id_user !=', $this->session->userdata('id_user'))
                    ->group_end();
            }
        }
    ...
    public function max_kode($date)
    {
        ...
        $where = 'id=(SELECT max(id) FROM pu_notifikasi where SUBSTRING(kode_notifikasi, 2, 4) = ' . $formatted_date . ')';
        $this->db->where($where);
        $query = $this->db->get('pu_notifikasi');
        return $query;
    }
