...
class M_reimbust_kps extends CI_Model
{
    // Reimbust
    var $id = 'id';
    var $table = 'tbl_reimbust_kps'; //nama tabel dari database
    ...
    // Deklarasi
    var $table3 = 'tbl_deklarasi_kps'; //nama tabel dari database
    ...
    {
        $this->db->select('tbl_reimbust_kps.*, tbl_data_user.name'); // Memilih kolom dari kedua tabel
        $this->db->from($this->table);
        $this->db->join('tbl_data_user', 'tbl_data_user.id_user = tbl_reimbust_kps.id_user', 'left'); // JOIN dengan tabel tbl_user
            ...
            if ($_POST['search']['value']) // jika datatable mengirimkan pencarian dengan metode POST
            {
                ...
                if ($i === 0) // looping awal
                {
                    ...
                    if ($item == 'name') {
                        ...
                        $this->db->like('tbl_reimbust_kps.' . $item, $_POST['search']['value']);
                    }
                } else {
                    if ($item == 'name') {
                        $this->db->or_like('tbl_data_user.' . $item, $_POST['search']['value']);
                    } else {
                        $this->db->or_like('tbl_reimbust_kps.' . $item, $_POST['search']['value']);
                    }
                }
        ...
        if (!empty($_POST['status'])) {
            ...
            if ($_POST['status'] == 'on-process') {
                ...
                $this->db->where('app_status', 'waiting')
                    ...
                    ->or_where('tbl_reimbust_kps.id_user =' . $id_user_logged_in . ' AND app_status = "approved" AND app2_status = "waiting"')
                ...
                $this->db->where('app2_name = (SELECT name FROM tbl_data_user WHERE id_user = ' . $id_user_logged_in . ' AND app2_status = "revised")', NULL, FALSE)
                    ...
                    ->or_where('tbl_reimbust_kps.id_user =' . $id_user_logged_in . ' AND (app_status = "revised" OR app2_status = "revised")');
        ...
        if (!empty($_POST['tab'])) {
            if ($_POST['tab'] == 'personal') {
                $this->db->where('tbl_reimbust_kps.id_user', $this->session->userdata('id_user'));
            } elseif ($_POST['tab'] == 'employee') {
                $this->db->group_start()
                    ->where('tbl_reimbust_kps.app_name =', "(SELECT name FROM tbl_data_user WHERE id_user = " . $this->session->userdata('id_user') . ")", FALSE)
                    ->where('tbl_reimbust_kps.id_user !=', $this->session->userdata('id_user'))
                    ->or_where('tbl_reimbust_kps.app2_name =', "(SELECT name FROM tbl_data_user WHERE id_user = " . $this->session->userdata('id_user') . ") && tbl_reimbust_kps.app_status = 'approved'", FALSE)
                    ->where('tbl_reimbust_kps.id_user !=', $this->session->userdata('id_user'))
                    ->group_end();
            }
        }
    ...
    private function _get_datatables_query2()
    {
        ...
        $this->db->select('tbl_deklarasi_kps.*, tbl_data_user.name');
        $this->db->from($this->table3);
        $this->db->join('tbl_data_user', 'tbl_data_user.id_user = tbl_deklarasi_kps.id_pengaju', 'left');
        $id_user_logged_in = $this->session->userdata('id_user'); // Mengambil id_user dari sesi pengguna yang login
            ...
            if ($_POST['search']['value']) // jika datatable mengirimkan pencarian dengan metode POST
            {
                ...
                if ($i === 0) // looping awal
                {
                    ...
                    if ($item == 'name') {
                        ...
                        $this->db->like('tbl_deklarasi_kps.' . $item, $_POST['search']['value']);
                    }
                } else {
                    if ($item == 'name') {
                        $this->db->or_like('tbl_data_user.' . $item, $_POST['search']['value']);
                    } else {
                        $this->db->or_like('tbl_deklarasi_kps.' . $item, $_POST['search']['value']);
                    }
                }
    ...
    {
        $this->db->where('is_active', 1);
        $this->db->select('tbl_prepayment_kps.*, tbl_data_user.name'); // Memilih kolom dari kedua tabel
        $this->db->from($this->table4);
        $this->db->join('tbl_data_user', 'tbl_data_user.id_user = tbl_prepayment_kps.id_user', 'left'); // JOIN dengan tabel tbl_user
        $id_user_logged_in = $this->session->userdata('id_user'); // Mengambil id_user dari sesi pengguna yang login
        $this->db->where('tbl_prepayment_kps.id_user', $id_user_logged_in);
        $this->db->where('tbl_prepayment_kps.status', 'approved');
            ...
            if ($_POST['search']['value']) // jika datatable mengirimkan pencarian dengan metode POST
            {
                ...
                if ($i === 0) // looping awal
                {
                    ...
                    if ($item == 'name') {
                        ...
                        $this->db->like('tbl_prepayment_kps.' . $item, $_POST['search']['value']);
                    }
                } else {
                    if ($item == 'name') {
                        $this->db->or_like('tbl_data_user.' . $item, $_POST['search']['value']);
                    } else {
                        $this->db->or_like('tbl_prepayment_kps.' . $item, $_POST['search']['value']);
                    }
                }
        ...
        if (!empty($_POST['tab'])) {
            if ($_POST['tab'] == 'personal') {
                $this->db->where('tbl_prepayment_kps.id_user', $this->session->userdata('id_user'));
            } elseif ($_POST['tab'] == 'employee') {
                $this->db->group_start()
                    ->where('tbl_prepayment_kps.app_name =', "(SELECT name FROM tbl_data_user WHERE id_user = " . $this->session->userdata('id_user') . ")", FALSE)
                    ->where('tbl_prepayment_kps.id_user !=', $this->session->userdata('id_user'))
                    ->or_where('tbl_prepayment_kps.app2_name =', "(SELECT name FROM tbl_data_user WHERE id_user = " . $this->session->userdata('id_user') . ") && tbl_prepayment.app_status = 'approved'", FALSE)
                    ->where('tbl_prepayment_kps.id_user !=', $this->session->userdata('id_user'))
                    ->group_end();
            }
        }
    ...
    public function max_kode($date)
    {
        ...
        $where = 'id=(SELECT max(id) FROM tbl_reimbust_kps where SUBSTRING(kode_reimbust, 2, 4) = ' . $formatted_date . ')';
        $this->db->where($where);
        $query = $this->db->get('tbl_reimbust_kps');
        return $query;
    }
    ...
    public function delete($id)
    {
        ...
        $detail = $this->db->get_where('tbl_reimbust_detail_kps', ['reimbust_id' => $id])->result_array();
        ...
        if ($detail) {
            foreach ($detail as $deklarasi) {
                ...
                $this->db->update('tbl_deklarasi_kps', ['is_active' => 1], ['kode_deklarasi' => $deklarasi]);
            }
        }

        // Ambil data tbl_reimbust berdasarkan reimbust_id
        $reimbust = $this->db->get_where('tbl_reimbust_kps', ['id' => $id])->row_array();
        ...
        $this->db->update('tbl_prepayment_kps', ['is_active' => 1], ['kode_prepayment' => $prepayment]);
