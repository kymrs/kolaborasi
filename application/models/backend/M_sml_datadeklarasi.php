...
class M_sml_datadeklarasi extends CI_Model
{
    var $id = 'id';
    var $table = 'sml_deklarasi'; //nama tabel dari database
    ...
    public function max_kode($date)
    {
        ...
        $where = 'id=(SELECT max(id) FROM sml_deklarasi where SUBSTRING(kode_deklarasi, 2, 4) = ' . $formatted_date . ')';
        $this->db->where($where);
        $query = $this->db->get('sml_deklarasi');
        return $query;
    }
