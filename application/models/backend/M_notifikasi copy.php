        ...
        foreach ($tbl_notifikasi as $table) {
            ...
            if ($tabel_nama != '' || $tabel_nama != null) {
                ...
                if (in_array('app4_name', $fields) && in_array('app4_status', $fields)) {
                    // app4 hanya jika statusnya waiting
                    $where[] = "(LOWER(app4_name) = " . $this->db->escape(strtolower($name)) . " AND app4_status = 'waiting')";
                }

                if (in_array('app_name', $fields) && in_array('app_status', $fields)) {
                    if (in_array('app4_status', $fields)) {
                        // app_name hanya jika app4 sudah approved
                        $where[] = "(LOWER(app_name) = " . $this->db->escape(strtolower($name)) . " AND app_status = 'waiting' AND app4_status = 'approved')";
                    } else {
                        $where[] = "(LOWER(app_name) = " . $this->db->escape(strtolower($name)) . " AND app_status = 'waiting')";
                    }
                }
