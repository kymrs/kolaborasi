...
class M_ctz_reimbust extends CI_Model
{
    // Reimbust
    var $id = 'id';
    var $table = 'ctz_reimbust'; //nama tabel dari database
    var $table2 = 'ctz_reimbust_detail';
    ...
    // Deklarasi
    var $table3 = 'ctz_deklarasi'; //nama tabel dari database
    ...
    {
        $this->db->select('ctz_reimbust.*, tbl_data_user.name'); // Memilih kolom dari kedua tabel
        $this->db->from($this->table);
        $this->db->join('tbl_data_user', 'tbl_data_user.id_user = ctz_reimbust.id_user', 'left'); // JOIN dengan tabel tbl_user
            ...
            if ($_POST['search']['value']) // jika datatable mengirimkan pencarian dengan metode POST
            {
                ...
                if ($i === 0) // looping awal
                {
                    ...
                    if ($item == 'name') {
                        ...
                        $this->db->like('ctz_reimbust.' . $item, $_POST['search']['value']);
                    }
                } else {
                    if ($item == 'name') {
                        $this->db->or_like('tbl_data_user.' . $item, $_POST['search']['value']);
                    } else {
                        $this->db->or_like('ctz_reimbust.' . $item, $_POST['search']['value']);
                    }
                }
        ...
        if (!empty($_POST['status'])) {
            ...
            if ($_POST['status'] == 'on-process') {
                if ($alias != "eko") {
                    $this->db
                        ->group_start()
                        ->where('ctz_reimbust.id_user', $id_user_logged_in)
                ...
                if ($alias != "eko") {
                    $this->db
                        ...
                        ->or_where('ctz_reimbust.id_user =' . $id_user_logged_in . ' AND (app4_status = "revised" OR app_status = "revised" OR app2_status = "revised")');
                } else {
        ...
        if (!empty($_POST['tab'])) {
            if ($_POST['tab'] == 'personal') {
                $this->db->where('ctz_reimbust.id_user', $this->session->userdata('id_user'));
            } elseif ($_POST['tab'] == 'employee') {
                if ($alias != "eko") {
                    $this->db->group_start()
                        ->where('ctz_reimbust.app_name =', "(SELECT name FROM tbl_data_user WHERE id_user = " . $this->session->userdata('id_user') . ")", FALSE)
                        ->where('ctz_reimbust.id_user !=', $this->session->userdata('id_user'))
                        ->or_where('ctz_reimbust.app2_name =', "(SELECT name FROM tbl_data_user WHERE id_user = " . $this->session->userdata('id_user') . ") && ctz_reimbust.app_status = 'approved'", FALSE)
                        ->where('ctz_reimbust.id_user !=', $this->session->userdata('id_user'))
                        ->or_where('ctz_reimbust.app4_name =', "(SELECT name FROM tbl_data_user WHERE id_user = " . $this->session->userdata('id_user') . ")", FALSE)
                        ->where('ctz_reimbust.id_user !=', $this->session->userdata('id_user'))
                        ->group_end();
                }
            }
    ...
    private function _get_datatables_query2()
    {
        ...
        $this->db->select('ctz_deklarasi.*, tbl_data_user.name');
        $this->db->from($this->table3);
        $this->db->join('tbl_data_user', 'tbl_data_user.id_user = ctz_deklarasi.id_pengaju', 'left');
        $id_user_logged_in = $this->session->userdata('id_user'); // Mengambil id_user dari sesi pengguna yang login
            ...
            if ($_POST['search']['value']) // jika datatable mengirimkan pencarian dengan metode POST
            {
                ...
                if ($i === 0) // looping awal
                {
                    ...
                    if ($item == 'name') {
                        ...
                        $this->db->like('ctz_deklarasi.' . $item, $_POST['search']['value']);
                    }
                } else {
                    if ($item == 'name') {
                        $this->db->or_like('tbl_data_user.' . $item, $_POST['search']['value']);
                    } else {
                        $this->db->or_like('ctz_deklarasi.' . $item, $_POST['search']['value']);
                    }
                }
            ...
            if ($_POST['search']['value']) // jika datatable mengirimkan pencarian dengan metode POST
            {
                ...
                if ($i === 0) // looping awal
                {
                    ...
                    if ($item == 'name') {
                        ...
                        $this->db->like('ctz_prepayment.' . $item, $_POST['search']['value']);
                    }
                } else {
                    if ($item == 'name') {
                        $this->db->or_like('tbl_data_user.' . $item, $_POST['search']['value']);
                    } else {
                        $this->db->or_like('ctz_prepayment.' . $item, $_POST['search']['value']);
                    }
                }
        ...
        if (!empty($_POST['tab'])) {
            if ($_POST['tab'] == 'personal') {
                ...
                $this->db->group_start()
                    ...
                    ->or_where('ctz_prepayment.app2_name =', "(SELECT name FROM tbl_data_user WHERE id_user = " . $this->session->userdata('id_user') . ") && sw_prepayment.app_status = 'approved'", FALSE)
                    ->where('ctz_prepayment.id_user !=', $this->session->userdata('id_user'))
                    ->group_end();
            }
        }
    ...
    public function max_kode($date)
    {
        ...
        $where = 'id=(SELECT max(id) FROM ctz_reimbust where SUBSTRING(kode_reimbust, 2, 4) = ' . $formatted_date . ')';
        $this->db->where($where);
        $query = $this->db->get('ctz_reimbust');
        return $query;
    }
    ...
    {
        // Ambil data ctz_reimbust_detail berdasarkan reimbust_id
        $detail = $this->db->get_where('ctz_reimbust_detail', ['reimbust_id' => $id])->result_array();

        // untuk menghapus file gambar
        if ($detail) {
            foreach ($detail as $rd) {
                $old_image = $rd['kwitansi'];
                $file_path = FCPATH . './assets/backend/document/reimbust/kwitansi/kwitansi_ctz/' . $old_image;
        ...
        if ($detail) {
            foreach ($detail as $deklarasi) {
                ...
                $this->db->update('ctz_deklarasi', ['is_active' => 1], ['kode_deklarasi' => $deklarasi]);
            }
        }

        // Ambil data ctz_reimbust berdasarkan reimbust_id
        ...
        $this->db->update('ctz_prepayment', ['is_active' => 1], ['kode_prepayment' => $prepayment]);
        ...
        $this->db->where('reimbust_id', $id);
        $this->db->delete('ctz_reimbust_detail');
    ...
    {
        return $this->db->distinct()->select('no_rek')->where('id_user', $id)->from('ctz_reimbust')->get();
    }
}
