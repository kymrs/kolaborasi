...
class M_sam_mom extends CI_Model
{
    var $id = 'id';
    var $table = 'sam_mom'; //nama tabel dari database
    ...
    public function max_kode()
    {
        ...
        $where = 'id=(SELECT max(id) FROM sam_mom where SUBSTRING(no_dok, 4, 4) = ' . date('ym') . ')';
        $this->db->where($where);
        $query = $this->db->get('sam_mom');
        return $query;
    }
