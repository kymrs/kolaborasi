...
class M_pu_reimbust extends CI_Model
{
    // Reimbust
    var $id = 'id';
    var $table = 'pu_reimbust'; //nama tabel dari database
    ...
    // Deklarasi
    var $table3 = 'pu_deklarasi'; //nama tabel dari database
    ...
    {
        $this->db->select('pu_reimbust.*, tbl_data_user.name'); // Memilih kolom dari kedua tabel
        $this->db->from($this->table);
        $this->db->join('tbl_data_user', 'tbl_data_user.id_user = pu_reimbust.id_user', 'left'); // JOIN dengan tabel tbl_user
            ...
            if ($_POST['search']['value']) // jika datatable mengirimkan pencarian dengan metode POST
            {
                ...
                if ($i === 0) // looping awal
                {
                    ...
                    if ($item == 'name') {
                        ...
                        $this->db->like('pu_reimbust.' . $item, $_POST['search']['value']);
                    }
                } else {
                    if ($item == 'name') {
                        $this->db->or_like('tbl_data_user.' . $item, $_POST['search']['value']);
                    } else {
                        $this->db->or_like('pu_reimbust.' . $item, $_POST['search']['value']);
                    }
                }
            ...
            if ($_POST['status'] == 'on-process') {
                ...
                if ($alias != "eko") {
                    $this->db->where('app_status', 'waiting')
                        ...
                        ->or_where('pu_reimbust.id_user =' . $id_user_logged_in . ' AND app_status = "approved" AND app2_status = "waiting"')
                ...
                if ($alias != "eko") {
                    $this->db->where('app2_name = (SELECT name FROM tbl_data_user WHERE id_user = ' . $id_user_logged_in . ' AND app2_status = "revised")', NULL, FALSE)
                        ...
                        ->or_where('pu_reimbust.id_user =' . $id_user_logged_in . ' AND (app_status = "revised" OR app2_status = "revised")');
                } else {
        ...
        if (!empty($_POST['tab'])) {
            if ($_POST['tab'] == 'personal') {
                $this->db->where('pu_reimbust.id_user', $this->session->userdata('id_user'));
            } elseif ($_POST['tab'] == 'employee') {
                if ($alias != "eko") {
                    $this->db->group_start()
                        ->where('pu_reimbust.app_name =', "(SELECT name FROM tbl_data_user WHERE id_user = " . $this->session->userdata('id_user') . ")", FALSE)
                        ->where('pu_reimbust.id_user !=', $this->session->userdata('id_user'))
                        ->or_where('pu_reimbust.app2_name =', "(SELECT name FROM tbl_data_user WHERE id_user = " . $this->session->userdata('id_user') . ") && pu_reimbust.app_status = 'approved'", FALSE)
                        ->where('pu_reimbust.id_user !=', $this->session->userdata('id_user'))
                        ->group_end();
                }
            }
    ...
    private function _get_datatables_query2()
    {
        ...
        $this->db->select('pu_deklarasi.*, tbl_data_user.name');
        $this->db->from($this->table3);
        $this->db->join('tbl_data_user', 'tbl_data_user.id_user = pu_deklarasi.id_pengaju', 'left');
        $id_user_logged_in = $this->session->userdata('id_user'); // Mengambil id_user dari sesi pengguna yang login
            ...
            if ($_POST['search']['value']) // jika datatable mengirimkan pencarian dengan metode POST
            {
                ...
                if ($i === 0) // looping awal
                {
                    ...
                    if ($item == 'name') {
                        ...
                        $this->db->like('pu_deklarasi.' . $item, $_POST['search']['value']);
                    }
                } else {
                    if ($item == 'name') {
                        $this->db->or_like('tbl_data_user.' . $item, $_POST['search']['value']);
                    } else {
                        $this->db->or_like('pu_deklarasi.' . $item, $_POST['search']['value']);
                    }
                }
    ...
    {
        $this->db->where('is_active', 1);
        $this->db->select('pu_prepayment.*, tbl_data_user.name'); // Memilih kolom dari kedua tabel
        $this->db->from($this->table4);
        $this->db->join('tbl_data_user', 'tbl_data_user.id_user = pu_prepayment.id_user', 'left'); // JOIN dengan tabel tbl_user
        $id_user_logged_in = $this->session->userdata('id_user'); // Mengambil id_user dari sesi pengguna yang login
        $this->db->where('pu_prepayment.id_user', $id_user_logged_in);
        $this->db->where('pu_prepayment.status', 'approved');
            ...
            if ($_POST['search']['value']) // jika datatable mengirimkan pencarian dengan metode POST
            {
                ...
                if ($i === 0) // looping awal
                {
                    ...
                    if ($item == 'name') {
                        ...
                        $this->db->like('pu_prepayment.' . $item, $_POST['search']['value']);
                    }
                } else {
                    if ($item == 'name') {
                        $this->db->or_like('tbl_data_user.' . $item, $_POST['search']['value']);
                    } else {
                        $this->db->or_like('pu_prepayment.' . $item, $_POST['search']['value']);
                    }
                }
        ...
        if (!empty($_POST['tab'])) {
            if ($_POST['tab'] == 'personal') {
                $this->db->where('pu_prepayment.id_user', $this->session->userdata('id_user'));
            } elseif ($_POST['tab'] == 'employee') {
                $this->db->group_start()
                    ->where('pu_prepayment.app_name =', "(SELECT name FROM tbl_data_user WHERE id_user = " . $this->session->userdata('id_user') . ")", FALSE)
                    ->where('pu_prepayment.id_user !=', $this->session->userdata('id_user'))
                    ->or_where('pu_prepayment.app2_name =', "(SELECT name FROM tbl_data_user WHERE id_user = " . $this->session->userdata('id_user') . ") && sw_prepayment.app_status = 'approved'", FALSE)
                    ->where('pu_prepayment.id_user !=', $this->session->userdata('id_user'))
                    ->group_end();
            }
        }
    ...
    public function max_kode($date)
    {
        ...
        $where = 'id=(SELECT max(id) FROM pu_reimbust where SUBSTRING(kode_reimbust, 2, 4) = ' . $formatted_date . ')';
        $this->db->where($where);
        $query = $this->db->get('pu_reimbust');
        return $query;
    }
    ...
    {
        // Ambil data pu_reimbust_detail berdasarkan reimbust_id
        $detail = $this->db->get_where('pu_reimbust_detail', ['reimbust_id' => $id])->result_array();

        // untuk menghapus file gambar
        if ($detail) {
            foreach ($detail as $rd) {
                $old_image = $rd['kwitansi'];
                $file_path = FCPATH . './assets/backend/document/reimbust/kwitansi/kwitansi_pu/' . $old_image;
        ...
        if ($detail) {
            foreach ($detail as $deklarasi) {
                ...
                $this->db->update('pu_deklarasi', ['is_active' => 1], ['kode_deklarasi' => $deklarasi]);
            }
        }

        // Ambil data pu_reimbust berdasarkan reimbust_id
        ...
        $this->db->update('pu_prepayment', ['is_active' => 1], ['kode_prepayment' => $prepayment]);
    ...
    {
        return $this->db->distinct()->select('no_rek')->where('id_user', $id)->from('pu_reimbust')->get();
    }
}
