        ...
        <div class="col-lg-12">
            <div class="card shadow mb-4">
                <div class="card-header text-right">
                    <a class="btn btn-secondary btn-sm" href="<?= base_url('sam_prepayment') ?>"><i class="fas fa-chevron-left"></i>&nbsp;Back</a>
                </div>
                <div class="card-body">
                    <form id="form">
                        <div class="row">
...
<script>
    $('#tgl_prepayment').datepicker({
        ...
        onSelect: function(dateText) {
            ...
            $.ajax({
                url: "<?php echo site_url('sam_prepayment/generate_kode') ?>",
                type: "POST",
                data: {
                    "date": dateText
                },
    ...
    $(document).ready(function() {
        ...
        if (id == 0) {
            ...
            $.ajax({
                url: "<?php echo site_url('sam_prepayment/edit_data') ?>/" + id,
                type: "GET",
                dataType: "JSON",
                success: function(data) {
                    // moment.locale('id')
        ...
        if (aksi == "read") {
            ...
            $.ajax({
                url: "<?php echo site_url('sam_prepayment/read_detail/') ?>" + id,
                type: "GET",
                dataType: "JSON",
                success: function(data) {
        ...
        $("#form").submit(function(e) {
            ...
            var url;
            if (id == 0) {
                url = "<?php echo site_url('sam_prepayment/add') ?>";
            } else {
                url = "<?php echo site_url('sam_prepayment/update') ?>";
            }
                ...
                success: function(data) {
                    ...
                    if (data.status) //if success close modal and reload ajax table
                    {
                        Swal.fire({
                            ...
                            location.href = "<?= base_url('sam_prepayment') ?>";
                        })
                    } else {
                        // Sembunyikan loading saat respons diterima
                        $('#loading').hide();
    ...
    //             $.ajax({
    //                 url: "<?php echo site_url('sam_prepayment/delete_event/') ?>" + id,
    //                 type: "POST",
    //                 dataType: "JSON",
    //                 success: function(data) {
    //                     Swal.fire({
    ...
    //                         timer: 1500
    //                     }).then((result) => {
    //                         location.href = "<?= base_url('sam_prepayment/add_form') ?>";
    //                     })
    //                 },
    //                 error: function(jqXHR, textStatus, errorThrown) {
