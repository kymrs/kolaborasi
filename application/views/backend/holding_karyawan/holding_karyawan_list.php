        ...
        <div class="col-lg-12">
            <div class="card shadow mb-4">
                <div class="card-header py-3">
                    <?php if ($add == 'Y') { ?>
                        <a class="btn btn-primary btn-sm" href="<?= base_url('holding_karyawan/add_form') ?>">
                            <i class="fa fa-plus"></i>&nbsp;&nbsp;Add Data
                        </a>
                        <?php } ?>
                    <a class="btn btn-primary btn-sm ml-2" style="border-radius: 4px" href="<?= base_url('holding_karyawan/e_pkwt') ?>">
                        <i class="fa fa-list"></i>&nbsp;&nbsp;E-PKWT
                    </a>
                    <div style="float: right; display:flex; gap:10px; align-items:center;">
                        <select name="status_kerja" id="status_kerja" class="form-control form-control-sm btn-primary btn-header" style="padding: 5px; width: 140px;">
                            <option value="">Semua Status</option>
                            <option value="Aktif">Aktif</option>
                            <option value="Non Aktif">Tidak Aktif</option>
                        </select>
                        <!-- <label style="margin:0; color:#666; font-weight:600;">Unit Bisnis:</label>
                        <select name="filter_unit_bisnis" id="filter_unit_bisnis" class="form-control form-control-sm btn-primary btn-header" style="padding: 5px; width: 180px;">
                            ...
                            <option value="hayfa">hayfa</option>
                        </select> -->
                        ...
                        <select id="filter_unit_bisnis" class="form-control btn-primary" style="width:auto; display:inline-block; font-size: 14px; padding: 6px 7px; border-radius: 4px; margin-right: 12px">
                            ...
                            <option value="Koperasi Nusantara Madani Sejahtera">Koperasi Nusantara Madani Sejahtera</option>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        table = $('#karyawan-table').DataTable({
            ...
            "order": [],    
            "ajax": {
                "url": "<?php echo site_url('holding_karyawan/get_list') ?>",
                "type": "POST",
                "data": function(data) {
                    data.status_kerja = $('#status_kerja').val(); // Mengambil nilai dari dropdown status kerja
                    data.filter_unit_bisnis = $('#filter_unit_bisnis').val(); // Mengambil nilai filter unit bisnis
                }
            },
            "language": {
                "infoFiltered": ""
    ...
    $('#status_kerja, #filter_unit_bisnis').on('change', function() {
    ...
    function delete_data(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "<?php echo site_url('holding_karyawan/delete_data_karyawan') ?>/" + id,
                    type: "POST",
                    dataType: "JSON",
                    success: function(data) {
                        Swal.fire({
