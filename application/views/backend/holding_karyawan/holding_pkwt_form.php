...
<script>
    (function(){
        ...
        $(document).off('input change', '#total_bulan').on('input change', '#total_bulan', function(){
            ...
            if (typeof generateNoPerjanjian === 'function') {
                generateNoPerjanjian();
            }
        });
...
<script>
    $('#npk').off('change').on('change', function() {
        ...
        $.ajax({
            url: '<?= site_url("holding_karyawan/get_karyawan_dates") ?>',
            method: 'POST',
            dataType: 'json',
            data: { npk: npk },
            success: function(res) {
                ...
                $.ajax({
                    url: '<?= site_url("holding_karyawan/get_contract_summary") ?>',
                    method: 'POST',
                    dataType: 'json',
                    data: { npk: npk },
    ...
    function generateNoPerjanjian() {
        ...
        if (npk && jk_awal && jk_akhir) {
            $.ajax({
                url: '<?= site_url("holding_karyawan/generate_no_perjanjian") ?>',
                type: 'POST',
                data: {
                    npk: npk,
                    jk_awal: jk_awal,
    ...
    $(document).ready(function() {
        ...
        if (id == 0) {
            ...
            $.ajax({
                url: "<?php echo site_url('holding_karyawan/edit_data2') ?>/" + id,
                type: "GET",
                dataType: "JSON",
        ...
        $("#form").submit(function(e) {
            ...
            if (id == 0) {
                ...
                url = "<?php echo site_url('holding_karyawan/update_kontrak_karyawan/') ?>" + id;
            }
                ...
                success: function(data) {
                    ...
                    if (data.status) {
                        Swal.fire({
                            ...
                            location.href = "<?= base_url('holding_karyawan/e_pkwt') ?>";
                        });
                    }
                },
                error: function(jqXHR, textStatus, errorThrown) {
