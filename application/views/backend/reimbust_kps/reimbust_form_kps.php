        ...
        <div class="col-lg-12">
            <div class="card shadow mb-4">
                <div class="card-header text-right">
                    <a class="btn btn-primary btn-sm btn-style" href="<?= base_url('reimbust_kps') ?>"><i class="fas fa-chevron-left"></i>&nbsp;Back</a>
                </div>
                <div class="card-body">
                            ...
                            <div class="modal-dialog modal-xl modal-dialog-scrollable">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        ...
                                        <!-- <a style="position: relative; right: 75px" class="btn btn-primary btn-sm" href="<?= base_url('datadeklarasi_kps    /add_form') ?>"><i class="fa fa-plus"></i>&nbsp;Add Data</a> -->
                                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
...
<script>
    $('#tgl_pengajuan').datepicker({
        ...
        onSelect: function(dateText) {
            ...
            $.ajax({
                url: "<?php echo site_url('reimbust_kps/generate_kode') ?>",
                type: "POST",
                data: {
                    "date": dateText
                },
    ...
    $(document).ready(function() {
        ...
        var table = $('#prepayment-table').DataTable({
            ...
            "order": [],
            "ajax": {
                "url": "<?php echo site_url('reimbust_kps/get_list3') ?>",
                "type": "POST",
                "data": function(d) {
                    d.status = 'approved';
                }
    ...
    $(document).ready(function() {
        var table = $('#deklarasi-table').DataTable({
            ...
            "order": [],
            "ajax": {
                "url": "<?php echo site_url('reimbust_kps/get_list2') ?>",
                "type": "POST"
            },
            "columnDefs": [{
                    "targets": [2],
    ...
    $(document).ready(function() {
        var table = $('#deklarasi-table-read').DataTable({
            ...
            "order": [],
            "ajax": {
                "url": "<?php echo site_url('datadeklarasi_kps/get_list') ?>",
                "type": "POST"
            },
            "columnDefs": [{
                    "targets": [2, 3, 4, 6],
    ...
    $(document).ready(function() {
        ...
        if (id == 0) {
            ...
            $.ajax({
                url: "<?= site_url('reimbust_kps/edit_data') ?>/" + id,
                type: "GET",
                dataType: "JSON",
                success: function(data) {
                    moment.locale('id')
                        ...
                        $(data['transaksi']).each(function(index) {
                            ...
                            const row = `
                                <tr id="row-${index + 1}">
                                    ...
                                    <td width="150" style="padding: 15px 10px">
                                        <a href="<?= base_url() ?>datadeklarasi_kps/read_form/25" class="btn btn-primary btn-lg btn-block btn-sm" 
                                            data-id="${index + 1}"
        ...
        $("#form").submit(function(e) {
            ...
            var url;
            if (id == 0) {
                url = "<?php echo site_url('reimbust_kps/add') ?>";
            } else {
                url = "<?php echo site_url('reimbust_kps/update') ?>";
            }
                ...
                success: function(data) {
                    ...
                    if (data.status) {
                        Swal.fire({
                            ...
                            location.href = "<?= base_url('reimbust_kps') ?>";
                        });
                    } else {
                        // Sembunyikan loading saat respons diterima
                        $('#loading').hide();
