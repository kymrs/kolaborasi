        ...
        <div class="col-lg-12">
            <div class="card shadow mb-4">
                <div class="card-header text-right">
                    <a class="btn btn-secondary btn-sm" href="<?= base_url('sam_mom') ?>"><i class="fas fa-chevron-left"></i>&nbsp;Back</a>
                </div>
                <div class="card-body">
                    <form id="form">
                        <div class="row">
...
<script>
    $(document).ready(function() {
        ...
        if (id == 0) {
            ...
            $('#lihat_foto').hide();
            $.ajax({
                url: "<?php echo site_url('sam_mom/edit_data') ?>/" + id,
                type: "GET",
                dataType: "JSON",
                success: function(data) {
                    ...
                    $('#lihat_foto').attr({
                        "href": "<?= site_url('assets/backend/document/mom/foto_mom_sam/') ?>" + data.foto,
                        "target": "_blank"
                    });
                },
        ...
        $("#form").submit(function(e) {
            ...
            var url;
            if (id == 0) {
                url = "<?php echo site_url('sam_mom/add') ?>";
            } else {
                url = "<?php echo site_url('sam_mom/update') ?>";
            }
                    ...
                    success: function(data) {
                        if (data.status) //if success close modal and reload ajax table
                        {
                            Swal.fire({
                                ...
                                location.href = "<?= base_url('sam_mom') ?>";
                            })
                        }
                    },
                    error: function(jqXHR, textStatus, errorThrown) {
