    ...
    <script>
        $(document).ready(function() {
            ...
            $('#appBtn').click(function() {
                ...
                $('#approvalForm').attr('action', '<?= site_url('reimbust_qbg/approve') ?>');
                $.ajax({
                    url: "<?php echo site_url('reimbust_qbg/edit_data') ?>/" + id,
                    type: "GET",
                    dataType: "JSON",
            ...
            $('#appBtn2').click(function() {
                ...
                $('#approvalForm').attr('action', '<?= site_url('reimbust_qbg/approve2') ?>');
            ...
            $('#paymentBtn').click(function() {
                $('#paymentForm').attr('action', '<?= site_url('reimbust_qbg/payment') ?>');
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (data['master']['app_status'] != null) {
                        ...
                        status = data['master']['app_status'];
                        keterangan = data['master']['app_keterangan'];
                        url = "<?php echo site_url('reimbust_qbg/approve') ?>";
                        $('#note_id').append(`<p>* ${keterangan}</p>`);
                    }
                    ...
                    $('.openModal').on('click', function() {
                        ...
                        if (kwitansi) {
                            ...
                            modalImg.attr('src', `<?= base_url() ?>/assets/backend/document/reimbust/kwitansi_qbg/${kwitansi}`);
                    ...
                    $(document).on('click', '[data-deklarasi]', function() {
                        ...
                        $.ajax({
                            url: '<?= site_url('reimbust_qbg/detail_deklarasi') ?>', // URL method controller
                            method: 'POST',
                            data: {
                                deklarasi: deklarasi
                            },
