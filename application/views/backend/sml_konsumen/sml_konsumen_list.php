            ...
            <div class="card shadow mb-4">
                ...
                <div class="card-body p-4">
                    ...
                    <div class="table-responsive">
                        <!-- Table wrapper -->
                        <table id="konsumen-table" class="table table-bordered table-striped display nowrap w-100 mb-4">
                            <!-- Added margin-bottom -->
                            <thead>
                                <tr>
                                    <th>No</th>
                ...
                <form id="modalform">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group row">
                            <label for="" class="col-sm-4 col-form-label">Nama Konsumen</label>
                            <div class="col-sm-8">
                                <input type="text" class="form-control" id="nama" name="nama" placeholder="Nama Konsumen" required>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        table = $('#konsumen-table').DataTable({
            "responsive": true,
            "scrollX": true,
            "processing": true,
            "serverSide": true,
            "order": [],
            "ajax": {
                "url": "<?php echo site_url('sml_konsumen/get_list') ?>",
                "type": "POST",
            },
            "language": {
                "infoFiltered": ""
    ...
    $("#modalform").submit(function(e) {
        ...
        if (method == 'add') {
            url = "<?php echo site_url('sml_konsumen/add') ?>";
        } else {
            url = "<?php echo site_url('sml_konsumen/update') ?>";
        }
    ...
    function edit_data(id) {
        ...
        $('.card-title').text('Edit Data Konsumen');
        $('.aksi').text('Update');
        $.ajax({
            url: "<?php echo site_url('sml_konsumen/get_id/') ?>/" + id,
            type: "GET",
            dataType: "JSON",
            success: function(data) {
                $('[name="id"]').val(data.id);
    ...
    function delete_data(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "<?php echo site_url('sml_konsumen/delete') ?>/" + id,
                    type: "POST",
                    dataType: "JSON",
                    success: function(data) {
                        Swal.fire({
