...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        table = $('#hotel-table').DataTable({
            ...
            "order": [],
            "ajax": {
                "url": "<?php echo site_url('nav_sop/get_list') ?>",
                "type": "POST",
                "data": function(d) {
    ...
    function loadParentOptions(jenis, selectedValue = null) {
        $.ajax({
            url: '<?php echo site_url('nav_sop/get_parent_options'); ?>',
            type: 'POST',
            dataType: 'JSON',
            data: {
                jenis: jenis
    ...
    $(document).on('select2:select', '#parent_no', function() {
        ...
        if (method === 'update') {
            $.ajax({
                url: '<?php echo site_url('nav_sop/generate_no_from_parent'); ?>',
                type: 'POST',
                dataType: 'JSON',
                data: {
                    jenis: jenis,
    ...
    $("#modalform").submit(function(e) {
        ...
        if (method == 'add') {
            url = "<?php echo site_url('nav_sop/add') ?>";
        } else {
            url = "<?php echo site_url('nav_sop/update') ?>";
        }
    ...
    function previewFile(filename) {
        ...
        var fileUrl = baseUrl + "assets/backend/document/sop/nav_sop/" + filename;
    ...
    function edit_data(id) {
        ...
        $('.aksi').text('Update');
        $.ajax({
            url: "<?php echo site_url('nav_sop/get_id/') ?>/" + id,
            type: "GET",
            dataType: "JSON",
            success: function(data) {
                $('[name="id"]').val(data.id);
    ...
    function delete_data(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "<?php echo site_url('nav_sop/delete') ?>/" + id,
                    type: "POST",
                    dataType: "JSON",
                    success: function(data) {
                        Swal.fire({
