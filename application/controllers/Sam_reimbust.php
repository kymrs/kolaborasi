...
class Sam_reimbust extends CI_Controller
{
    public function __construct()
    {
        parent::__construct();
        $this->load->model('backend/M_sam_reimbust');
    ...
    public function index()
    {
        ...
        $data['title'] = "backend/sam_reimbust/sam_reimbust_list";
    ...
    function get_list()
    {
        ...
        $fullname = $this->db->select('name')
            ...
            ->get()
            ->row('name');
        $list = $this->M_sam_reimbust->get_datatables();
        $data = array();
        $no = $_POST['start'];
        ...
        foreach ($list as $field) {
            ...
            $action_read = ($read == 'Y') ? '<a href="sam_reimbust/read_form/' . $field->id . '" class="btn btn-info btn-circle btn-sm" title="Read"><i class="fa fa-eye"></i></a>&nbsp;' : '';
            $action_edit = ($edit == 'Y') ? '<a href="sam_reimbust/edit_form/' . $field->id . '" class="btn btn-warning btn-circle btn-sm" title="Edit"><i class="fa fa-edit"></i></a>&nbsp;' : '';
            $action_delete = ($delete == 'Y') ? '<a onclick="delete_data(' . "'" . $field->id . "'" . ')" class="btn btn-danger btn-circle btn-sm" title="Delete"><i class="fa fa-trash"></i></a>&nbsp;' : '';
            $action_print = ($print == 'Y') ? '<a class="btn btn-success btn-circle btn-sm" target="_blank" href="sam_reimbust/generate_pdf/' . $field->id . '"><i class="fas fa-file-pdf"></i></a>' : '';
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->M_sam_reimbust->count_all(),
            "recordsFiltered" => $this->M_sam_reimbust->count_filtered(),
            "data" => $data,
        );
    ...
    function get_list2()
    {
        ...
        $fullname = $this->db->select('name')
            ...
            ->get()
            ->row('name');
        $list = $this->M_sam_reimbust->get_datatables2();
        $data = array();
        $no = $_POST['start'];
        ...
        foreach ($list as $field) {
            ...
            if ($field->app_name == $fullname) {
                $action = '<a href="sam_datadeklarasi/read_form/' . $field->id . '" class="btn btn-info btn-circle btn-sm" title="Read"><i class="fa fa-eye"></i></a>
                                <a class="btn btn-success btn-circle btn-sm" href="sam_datadeklarasi/generate_pdf/' . $field->id . '"><i class="fas fa-file-pdf"></i></a>';
                ...
                $action = '<a href="sam_datadeklarasi/read_form/' . $field->id . '" class="btn btn-info btn-circle btn-sm" title="Read"><i class="fa fa-eye"></i></a>
                    <a href="sam_datadeklarasi/edit_form/' . $field->id . '" class="btn btn-warning btn-circle btn-sm" title="Edit"><i class="fa fa-edit"></i></a>
                    <a class="btn btn-success btn-circle btn-sm" href="sam_datadeklarasi/generate_pdf/' . $field->id . '"><i class="fas fa-file-pdf"></i></a>';
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->M_sam_reimbust->count_all2(),
            "recordsFiltered" => $this->M_sam_reimbust->count_filtered2(),
            "data" => $data,
        );
        //output dalam format JSON
        echo json_encode($output);
    ...
    function get_list3()
    {
        ...
        $fullname = $this->db->select('name')
            ...
            ->get()
            ->row('name');
        $list = $this->M_sam_reimbust->get_datatables3();
        $data = array();
        $no = $_POST['start'];
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->M_sam_reimbust->count_all3(),
            "recordsFiltered" => $this->M_sam_reimbust->count_filtered3(),
            "data" => $data,
        );
        //output dalam format JSON
        echo json_encode($output);
    ...
    function read_form($id)
    {
        ...
        $data['aksi'] = 'read';
        $data['user'] = $this->M_sam_reimbust->get_by_id($id);
        ...
        $data['title'] = 'backend/sam_reimbust/sam_reimbust_read';
        $this->db->select('kwitansi');
        $this->db->where('reimbust_id', $id);
        $data['kwitansi'] = $this->db->get('sam_reimbust_detail')->result_array();
        $this->load->view('backend/home', $data);
    }
    ...
    public function add_form()
    {
        ...
        $data['id'] = 0;
        $data['aksi'] = 'add';
        $data['rek_options'] = $this->M_sam_reimbust->options($id)->result_array();
        $data['title_view'] = "Reimbust Form";
        $data['title'] = 'backend/sam_reimbust/sam_reimbust_form';
        $this->load->view('backend/home', $data);
    }
    ...
    public function generate_pdf($id)
    {
        ...
        $data['master'] = $this->M_sam_reimbust->get_by_id($id);
        $data['transaksi'] = $this->M_sam_reimbust->get_by_id_detail($id);
    ...
    public function generate_kode()
    {
        ...
        $kode = $this->M_sam_reimbust->max_kode($date)->row();
    ...
    function edit_form($id)
    {
        ...
        $data['id_pembuat'] = $this->M_sam_reimbust->get_by_id($id)->id_user;


        $data['id'] = $id;
        $data['aksi'] = 'update';
        $data['title_view'] = "Edit Reimbust";
        $data['rek_options'] = $this->M_sam_reimbust->options($data['id_user'])->result_array();
        $data['title'] = 'backend/sam_reimbust/sam_reimbust_form';
        $this->load->view('backend/home', $data);
    }
    ...
    {
        $data = $this->M_sam_reimbust->get_by_id_detail($id);
        echo json_encode($data);
    }
    ...
    public function detail_deklarasi()
    {
        if ($this->input->is_ajax_request()) {
            ...
            $deklarasiRecord = $this->db->get_where('sam_deklarasi', ['kode_deklarasi' => $deklarasi])->row_array();
            ...
            if ($deklarasiRecord) {
                ...
                $redirect_url = site_url('sam_datadeklarasi/read_form/' . $deklarasiId);
    ...
    public function add()
    {
        ...
        if ($deklarasiDataJson) {
            ...
            if (is_array($deklarasiData)) {
                foreach ($deklarasiData as $d) {
                    ...
                    $existingDeklarasi = $this->db->get_where('sam_deklarasi', ['kode_deklarasi' => $requestedCode])->row();
                    ...
                    if ($existingDeklarasi || $alreadyUsedInBatch || $isNotSequential) {
                        $finalCode = $this->generate_new_sam_deklarasi_code($d['tgl'], $used_deklarasi_codes);
        ...
        if ($valid) {
            $reimbust_id = $this->M_sam_reimbust->save($data1);
        }
        ...
        for ($i = 1; $i <= count($pemakaian); $i++) {
            ...
            if (!empty($_FILES['kwitansi']['name'][$i])) {
                ...
                $config['upload_path'] = './assets/backend/document/reimbust/kwitansi/kwitansi_sam';
        ...
        $this->db->update('sam_prepayment');

        $this->M_sam_reimbust->save_detail($data2);
     ...
     */
    private function generate_new_sam_deklarasi_code($date, &$used_codes = [])
    {
        ...
        while (true) {
            ...
            $exists = $this->db->get_where('sam_deklarasi', ['kode_deklarasi' => $candidate])->row();
            if ($exists) {
                $nextOrdinal++;
                continue;
            }
    ...
    public function update()
    {
        ...
        if ($deklarasiDataJson) {
            ...
            if (is_array($deklarasiData)) {
                foreach ($deklarasiData as $d) {
                    if (!empty($d['kode'])) {
                        $exists = $this->db->get_where('sam_deklarasi', ['kode_deklarasi' => $d['kode']])->row();
                        if (!$exists) {
        ...
        if ($this->db->update('sam_reimbust', $data)) {
            // 1. Hapus Baris yang Telah Dihapus
            $deletedRows = json_decode($this->input->post('deleted_rows'), true);
            if (!empty($deletedRows)) {
                foreach ($deletedRows as $id2) {
                    $reimbust_detail = $this->db->get_where('sam_reimbust_detail', ['id' => $id2])->row_array();

                    if ($reimbust_detail) {
                        $old_image = $reimbust_detail['kwitansi'];
                        if ($old_image != 'default.jpg') {
                            @unlink(FCPATH . './assets/backend/document/reimbust/kwitansi/kwitansi_sam/' . $old_image);
                        }

                        $this->db->where('id', $id2);
                        $this->db->delete('sam_reimbust_detail');

                        $kode_deklarasi = $reimbust_detail['deklarasi'];
                        $this->db->update('sam_deklarasi', ['is_active' => 1], ['kode_deklarasi' => $kode_deklarasi]);
                    }
                }
            }
            ...
            foreach ($pemakaian as $i => $p) {
                ...
                if (isset($_FILES['kwitansi']['name'][$i]) && !empty($_FILES['kwitansi']['name'][$i])) {
                    ...
                    if ($this->upload->do_upload('file')) {
                        ...
                        $reimbust_detail = $this->db->get_where('sam_reimbust_detail', ['id' => $id])->row_array();
                ...
                if (!empty($kode_prepayment)) {
                    $this->db->update('sam_prepayment', ['is_active' => 0], ['kode_prepayment' => $kode_prepayment]);
                }
                ...
                if ($kode_prepayment != $kode_prepayment_old && !empty($kode_prepayment_old)) {
                    $this->db->update('sam_prepayment', ['is_active' => 1], ['kode_prepayment' => $kode_prepayment_old]);
                }

                // Replace data di sam_reimbust_detail
                $this->db->replace('sam_reimbust_detail', $data2);
                ...
                if ($oldDeklarasi !== '' && $newDeklarasi !== '' && $oldDeklarasi !== $newDeklarasi) {
                    $this->db->update('sam_deklarasi', ['is_active' => 1], ['kode_deklarasi' => $oldDeklarasi]);
                    $this->db->update('sam_deklarasi', ['is_active' => 0], ['kode_deklarasi' => $newDeklarasi]);
    ...
    private function sync_deklarasi_approval_by_reimbust($reimbustId, array $deklarasiUpdate)
    {
        ...
        $this->db->update('sam_deklarasi', $deklarasiUpdate);
    }
    ...
    public function approve3()
    {
        ...
        $this->db->update('sam_reimbust', $data);
    ...
    public function payment()
    {
        ...
        if ($payment_status === 'paid' && !empty($tgl_pembayaran)) {
            ...
            $reimbust = $this->db->get_where('sam_reimbust', ['id' => $id])->row_array();

            if ($reimbust && $reimbust['attachment'] && $reimbust['attachment'] != 'default.pdf') {
                @unlink(FCPATH . './assets/backend/document/reimbust/attachment/sam_attachment/' . $reimbust['attachment']);
                }
        ...
        if (!empty($_FILES['attachment']['name'])) {
            if ($reimbust && !empty($reimbust['attachment']) && $reimbust['attachment'] != 'default.pdf') {
                $oldFile = FCPATH . 'assets/backend/document/reimbust/attachment/sam_attachment/' . $reimbust['attachment'];
        ...
        // Handle upload file
        if (!empty($_FILES['attachment']['name'])) {
            $config['upload_path'] = 'assets/backend/document/reimbust/attachment/sam_attachment/';
        ...
        // Update database
        $this->db->where('id', $id);
        $result = $this->db->update('sam_reimbust', $data);
        if ($result) {
            echo json_encode([
                "status" => TRUE
            ]);
    ...
    public function get_budget_transport()
    {
        $id_user = $this->session->userdata('id_user');
        $current_month = date('m');
        $current_year = date('Y');

        $total_used = $this->db->select('COALESCE(SUM(srd.jumlah), 0) as total')
            ->from('sam_reimbust_detail srd')
            ->join('sam_reimbust sr', 'srd.reimbust_id = sr.id')
            ->where('sr.id_user', $id_user)
            ->where('sr.sifat_pelaporan', 'Transport')
            ->where('MONTH(sr.tgl_pengajuan)', $current_month)
            ->where('YEAR(sr.tgl_pengajuan)', $current_year)
        ...
        $budget_limit = 750000;
        $sisa_budget = $budget_limit - $total_used;
        ...
        echo json_encode([
            ...
            'sisa_budget' => max(0, $sisa_budget),
            'can_submit' => $sisa_budget > 0
        ]);
    }
}
