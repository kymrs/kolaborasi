...
class Kps_sop extends CI_Controller
{
    ...
    public function add()
    {
        ...
        // Config upload
        $config['upload_path'] = './assets/backend/document/sop/kps_sop';
    ...
    public function update()
    {
        ...
        if (!empty($_FILES['file']['name'])) {
            if ($this->upload->do_upload('file')) {
                ...
                if (!empty($old_file) && file_exists('assets/backend/document/sop/kps_sop/' . $old_file)) {
                    unlink('assets/backend/document/sop/kps_sop/' . $old_file);
                }
            } else {
                // Upload failed
                $error = $this->upload->display_errors();
    ...
    public function delete($id)
    {
        ...
        if (!empty($file)) {
            ...
            $fullPath = rtrim(FCPATH, DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR . 'assets' . DIRECTORY_SEPARATOR . 'backend' . DIRECTORY_SEPARATOR . 'document' . DIRECTORY_SEPARATOR . 'sop' . DIRECTORY_SEPARATOR . 'kps_sop' . DIRECTORY_SEPARATOR . $safeFile;
            if (is_file($fullPath)) {
                @unlink($fullPath);
            }
        }
