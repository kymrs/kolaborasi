...
class Pu_data_agen extends CI_Controller
{
    ...
    {
        $this->M_login->getsecurity();
        $data['title'] = "backend/pu_data_agen/pu_data_member_list";
    ...
    function get_list2()
    {
        ...
        $list = $this->M_pu_data_agen->get_datatables2($status);
        $data = array();
        $no = $_POST['start'];
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->M_pu_data_agen->count_all2(),
            "recordsFiltered" => $this->M_pu_data_agen->count_filtered2(),
            "data" => $data,
        );
        //output dalam format JSON
        echo json_encode($output);
    ...
    public function update_payment_status_member()
    {
        ...
        if ($payment_status == 0 && $insentif->jenis_transaksi == 'pencairan') {
            $agen = $this->db->where('no_hp', $insentif->no_telp)->get('pu_customer')->row();
            if ($agen) {
