...
class Pu_data_agen extends CI_Controller
{
    ...
    public function update_saldo_agen()
    {
        $id = $this->input->post('id');
    ...
    public function api_data_agen()
    {
        ...
        $data = $this->db->select('id, nama, no_telp')->get('pu_data_agen')->result();
    ...
    public function login_agen()
    {
        ...
        // Normalisasi nomor
        $no_telp_alt = "+62" . ltrim($no_telp, "0");

        // Ambil user
        $this->db->where("(no_telp = '$no_telp' OR no_telp = '$no_telp_alt')", NULL, FALSE);
        $query = $this->db->select('id, nama, no_telp, kode_referral, saldo, password, is_active')->get('pu_data_agen');
        ...
        $agen = $query->row_array();

        // CEK PASSWORD
        if (!empty($agen['password'])) {
            // Bandingkan MD5
            if (md5($password) !== $agen['password']) {
                echo json_encode([
                    "status" => false,
                    "message" => "No. Telepon atau Password salah!"
                ]);
