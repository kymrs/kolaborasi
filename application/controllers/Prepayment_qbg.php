...
class Prepayment_by_moment extends CI_Controller
{
    public function __construct()
    {
        parent::__construct();
        $this->load->model('backend/M_prepayment_by_moment');
    ...
    public function index()
    {
        ...
        $data['title'] = "backend/prepayment_by_moment/prepayment_list_by_moment";
    ...
    {
        $this->load->view('backend/prepayment_by_moment/prepayment_pdf');
    }
    ...
    function get_list()
    {
        ...
        $fullname = $this->db->select('name')
            ...
            ->get()
            ->row('name');
        $list = $this->M_prepayment_by_moment->get_datatables();
        $data = array();
        $no = $_POST['start'];
        ...
        foreach ($list as $field) {
            ...
            $action_read = ($read == 'Y') ? '<a href="prepayment_by_moment/read_form/' . $field->id . '" class="btn btn-info btn-circle btn-sm" title="Read"><i class="fa fa-eye"></i></a>&nbsp;' : '';
            $action_edit = ($edit == 'Y') ? '<a href="prepayment_by_moment/edit_form/' . $field->id . '" class="btn btn-warning btn-circle btn-sm" title="Edit"><i class="fa fa-edit"></i></a>&nbsp;' : '';
            $action_delete = ($delete == 'Y') ? '<a onclick="delete_data(' . "'" . $field->id . "'" . ')" class="btn btn-danger btn-circle btn-sm" title="Delete"><i class="fa fa-trash"></i></a>&nbsp;' : '';
            $action_print = ($print == 'Y') ? '<a class="btn btn-success btn-circle btn-sm" target="_blank" href="prepayment_by_moment/generate_pdf/' . $field->id . '"><i class="fas fa-file-pdf"></i></a>' : '';
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->M_prepayment_by_moment->count_all(),
            "recordsFiltered" => $this->M_prepayment_by_moment->count_filtered(),
            "data" => $data,
        );
    ...
    public function read_form($id)
    {
        $data['notif'] = $this->M_notifikasi->pending_notification();
        $data['id'] = $id;
        $data['user'] = $this->M_prepayment_by_moment->get_by_id($id);
        ...
        $data['app2_name'] = $this->db->select('name')
            ...
            ->get()
            ->row('name');
        $data['title'] = 'backend/prepayment_by_moment/prepayment_read_by_moment';
    ...
    {
        $data['id'] = 0;
        $data['title'] = 'backend/prepayment_by_moment/prepayment_form_by_moment';
    ...
    public function generate_kode()
    {
        ...
        $kode = $this->M_prepayment_by_moment->max_kode($date)->row();
    ...
    {
        $data['master'] = $this->M_prepayment_by_moment->get_by_id($id);
        $data['transaksi'] = $this->M_prepayment_by_moment->get_by_id_detail($id);
    ...
    {
        $data = $this->M_prepayment_by_moment->get_by_id_detail($id);
        echo json_encode($data);
    }
    ...
    public function add()
    {
        ...
        $approval = $this->M_prepayment_by_moment->approval($this->session->userdata('id_user'));
        ...
        $inserted = $this->M_prepayment_by_moment->save($data);
        ...
        if ($inserted) {
            ...
            $this->M_prepayment_by_moment->save_detail($data2);
        }
    ...
    public function update()
    {
        ...
        $nominal = $this->input->post('hidden_nominal[]');
        $keterangan = $this->input->post('keterangan[]');
        if ($this->db->update('tbl_prepayment_by_moment', $data)) {
            ...
            if (!empty($deletedRows)) {
                foreach ($deletedRows as $id2) {
                    ...
                    $this->db->delete('tbl_prepayment_detail_by_moment');
                }
            }
            ...
            for ($i = 1; $i <= count($_POST['rincian']); $i++) {
                ...
                $this->db->replace('tbl_prepayment_detail_by_moment', $data2[$i - 1]);
            }
        }
        echo json_encode(array("status" => TRUE));
    }
    ...
    function delete($id)
    {
        $this->M_prepayment_by_moment->delete($id);
        $this->M_prepayment_by_moment->delete_detail($id);
        echo json_encode(array("status" => TRUE));
    }
    ...
    public function approve()
    {
        ...
        $this->db->update('tbl_prepayment_by_moment', $data);
    ...
    function signature()
    {
        ...
        if (file_by_momentt_contents($filePath, $data)) {
    ...
    {
        $this->db->where('id', $this->input->post('id'));
        $this->db->update('tbl_prepayment_by_moment', ['payment_status' => $this->input->post('payment_status')]);
