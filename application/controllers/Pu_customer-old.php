...
class pu_customer extends CI_Controller
{
    ...
    function get_list()
    {
        ...
        foreach ($list as $field) {
            ...
            $action_read = ($read == 'Y') ? '<a href="pu_customer/read_form/' . $field->id . '" class="btn btn-info btn-circle btn-sm" title="Read"><i class="fa fa-eye"></i></a>&nbsp;' : '';
            $action_edit = ($edit == 'Y') ? '<a href="pu_customer/edit_form/' . $field->id . '" class="btn btn-warning btn-circle btn-sm" title="Edit"><i class="fa fa-edit"></i></a>&nbsp;' : '';
    ...
    public function add()
    {
        ...
        if (empty($group_id_input)) {
            $last_group_id = $this->M_pu_customer->get_last_group_id(); // Fungsi untuk mendapatkan group_id terakhir
