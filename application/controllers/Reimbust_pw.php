...
class Reimbust_pw extends CI_Controller
{
    public function __construct()
    {
        parent::__construct();
        $this->load->model('backend/M_reimbust_pw');
    ...
    public function index()
    {
        ...
        $data['title'] = "backend/reimbust_pw/reimbust_list_pw";
    ...
    function get_list()
    {
        ...
        $fullname = $this->db->select('name')
            ...
            ->get()
            ->row('name');
        $list = $this->M_reimbust_pw->get_datatables();
        $data = array();
        $no = $_POST['start'];
        ...
        foreach ($list as $field) {
            ...
            $action_read = ($read == 'Y') ? '<a href="reimbust_pw/read_form/' . $field->id . '" class="btn btn-info btn-circle btn-sm" title="Read"><i class="fa fa-eye"></i></a>&nbsp;' : '';
            $action_edit = ($edit == 'Y') ? '<a href="reimbust_pw/edit_form/' . $field->id . '" class="btn btn-warning btn-circle btn-sm" title="Edit"><i class="fa fa-edit"></i></a>&nbsp;' : '';
            $action_delete = ($delete == 'Y') ? '<a onclick="delete_data(' . "'" . $field->id . "'" . ')" class="btn btn-danger btn-circle btn-sm" title="Delete"><i class="fa fa-trash"></i></a>&nbsp;' : '';
            $action_print = ($print == 'Y') ? '<a class="btn btn-success btn-circle btn-sm" target="_blank" href="reimbust_pw/generate_pdf/' . $field->id . '"><i class="fas fa-file-pdf"></i></a>' : '';
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->M_reimbust_pw->count_all(),
            "recordsFiltered" => $this->M_reimbust_pw->count_filtered(),
            "data" => $data,
        );
    ...
    function get_list2()
    {
        ...
        $fullname = $this->db->select('name')
            ...
            ->get()
            ->row('name');
        $list = $this->M_reimbust_pw->get_datatables2();
        $data = array();
        $no = $_POST['start'];
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->M_reimbust_pw->count_all2(),
            "recordsFiltered" => $this->M_reimbust_pw->count_filtered2(),
            "data" => $data,
        );
        //output dalam format JSON
        echo json_encode($output);
    ...
    function get_list3()
    {
        ...
        $fullname = $this->db->select('name')
            ...
            ->get()
            ->row('name');
        $list = $this->M_reimbust_pw->get_datatables3();
        $data = array();
        $no = $_POST['start'];
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->M_reimbust_pw->count_all3(),
            "recordsFiltered" => $this->M_reimbust_pw->count_filtered3(),
            "data" => $data,
        );
        //output dalam format JSON
        echo json_encode($output);
    ...
    function read_form($id)
    {
        $data['notif'] = $this->M_notifikasi->pending_notification();
        $data['aksi'] = 'read';
        $data['user'] = $this->M_reimbust_pw->get_by_id($id);
        ...
        $data['title'] = 'backend/reimbust_pw/reimbust_read_pw';
        $this->db->select('kwitansi');
        $this->db->where('reimbust_id', $id);
        $data['kwitansi'] = $this->db->get('tbl_reimbust_detail_pw')->result_array();
        $this->load->view('backend/home', $data);
    }
    ...
    public function add_form()
    {
        ...
        $data['title'] = 'backend/reimbust_pw/reimbust_form_pw';
        $this->load->view('backend/home', $data);
    }
    ...
    public function generate_pdf($id)
    {
        ...
        $data['master'] = $this->M_reimbust_pw->get_by_id($id);
        $data['transaksi'] = $this->M_reimbust_pw->get_by_id_detail($id);
    ...
    public function generate_kode()
    {
        ...
        $kode = $this->M_reimbust_pw->max_kode($date)->row();
    ...
    {
        $data = $this->M_reimbust_pw->get_by_id_detail($id);
        echo json_encode($data);
    }
    ...
    public function detail_deklarasi()
    {
        if ($this->input->is_ajax_request()) {
            ...
            $deklarasiRecord = $this->db->get_where('tbl_deklarasi_pw', ['kode_deklarasi' => $deklarasi])->row_array();
    ...
    public function add()
    {
        ...
        $approval = $this->M_reimbust_pw->approval($this->session->userdata('id_user'));
        ...
        if ($valid) {
            $reimbust_id = $this->M_reimbust_pw->save($data1);
        }
        ...
        for ($i = 1; $i <= count($pemakaian); $i++) {
            ...
            if (!empty($_FILES['kwitansi']['name'][$i])) {
                ...
                $config['upload_path'] = './assets/backend/document/reimbust/kwitansi_pw/';
            ...
            $this->db->update('tbl_deklarasi_pw');
        }
        $this->db->set('is_active', 0);
        $this->db->where('kode_prepayment', $this->input->post('kode_prepayment'));
        $this->db->update('tbl_prepayment_pw');

        $this->M_reimbust_pw->save_detail($data2);
    ...
    public function update()
    {
        ...
        if ($this->db->update('tbl_reimbust_pw', $data)) {
            // 1. Hapus Baris yang Telah Dihapus
            $deletedRows = json_decode($this->input->post('deleted_rows'), true);
            if (!empty($deletedRows)) {
                foreach ($deletedRows as $id2) {
                    $reimbust_detail = $this->db->get_where('tbl_reimbust_detail_pw', ['id' => $id2])->row_array();

                    if ($reimbust_detail) {
                        $old_image = $reimbust_detail['kwitansi'];
                        if ($old_image != 'default.jpg') {
                            @unlink(FCPATH . './assets/backend/document/reimbust/kwitansi_pw/' . $old_image);
                        }
            ...
            foreach ($pemakaian as $i => $p) {
                ...
                if (isset($_FILES['kwitansi']['name'][$i]) && !empty($_FILES['kwitansi']['name'][$i])) {
                    ...
                    if ($this->upload->do_upload('file')) {
                        ...
                        $reimbust_detail = $this->db->get_where('tbl_reimbust_detail_pw', ['id' => $id])->row_array();
    ...
    public function approve()
    {
        ...
        $this->db->update('tbl_reimbust_pw', $data);
    ...
    {
        $this->db->where('id', $this->input->post('id'));
        $this->db->update('tbl_reimbust_pw', ['payment_status' => $this->input->post('payment_status')]);
