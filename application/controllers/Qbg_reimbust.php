...
class Qbg_reimbust extends CI_Controller
{
    public function __construct()
    {
        parent::__construct();
        $this->load->model('backend/M_qbg_reimbust');
    ...
    function get_list()
    {
        ...
        $fullname = $this->db->select('name')
            ...
            ->get()
            ->row('name');
        $list = $this->M_qbg_reimbust->get_datatables();
        $data = array();
        $no = $_POST['start'];
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->M_qbg_reimbust->count_all(),
            "recordsFiltered" => $this->M_qbg_reimbust->count_filtered(),
            "data" => $data,
        );
    ...
    function get_list2()
    {
        ...
        $fullname = $this->db->select('name')
            ...
            ->get()
            ->row('name');
        $list = $this->M_qbg_reimbust->get_datatables2();
        $data = array();
        $no = $_POST['start'];
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->M_qbg_reimbust->count_all2(),
            "recordsFiltered" => $this->M_qbg_reimbust->count_filtered2(),
            "data" => $data,
        );
        //output dalam format JSON
        echo json_encode($output);
    ...
    function get_list3()
    {
        ...
        $fullname = $this->db->select('name')
            ...
            ->get()
            ->row('name');
        $list = $this->M_qbg_reimbust->get_datatables3();
        $data = array();
        $no = $_POST['start'];
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->M_qbg_reimbust->count_all3(),
            "recordsFiltered" => $this->M_qbg_reimbust->count_filtered3(),
            "data" => $data,
        );
        //output dalam format JSON
        echo json_encode($output);
    ...
    function read_form($id)
    {
        ...
        $data['aksi'] = 'read';
        $data['user'] = $this->M_qbg_reimbust->get_by_id($id);
    ...
    public function generate_pdf($id)
    {
        ...
        $data['master'] = $this->M_qbg_reimbust->get_by_id($id);
        $data['transaksi'] = $this->M_qbg_reimbust->get_by_id_detail($id);
    ...
    public function generate_kode()
    {
        ...
        $kode = $this->M_qbg_reimbust->max_kode($date)->row();
    ...
    function edit_form($id)
    {
        ...
        $data['id_pembuat'] = $this->M_qbg_reimbust->get_by_id($id)->id_user;
    ...
    {
        $data = $this->M_qbg_reimbust->get_by_id_detail($id);
        echo json_encode($data);
    }
    ...
    public function detail_deklarasi()
    {
        if ($this->input->is_ajax_request()) {
            ...
            $deklarasiRecord = $this->db->get_where('qbg_deklarasi', ['kode_deklarasi' => $deklarasi])->row_array();
            ...
            if ($deklarasiRecord) {
                ...
                $redirect_url = site_url('qbg_datadeklarasi/read_form/' . $deklarasiId);
    ...
    public function add()
    {
        ...
        if ($valid) {
            $reimbust_id = $this->M_qbg_reimbust->save($data1);
        }
        ...
        for ($i = 1; $i <= count($pemakaian); $i++) {
            ...
            if (!empty($_FILES['kwitansi']['name'][$i])) {
                ...
                $config['upload_path'] = './assets/backend/document/reimbust/kwitansi/kwitansi_qbg/';
            ...
            $this->db->update('qbg_deklarasi');
        }
        $this->db->set('is_active', 0);
        $this->db->where('kode_prepayment', $this->input->post('kode_prepayment'));
        $this->db->update('qbg_prepayment');

        $this->M_qbg_reimbust->save_detail($data2);
    ...
    public function update()
    {
        ...
        if ($this->db->update('qbg_reimbust', $data)) {
            // 1. Hapus Baris yang Telah Dihapus
            $deletedRows = json_decode($this->input->post('deleted_rows'), true);
            if (!empty($deletedRows)) {
                foreach ($deletedRows as $id2) {
                    $reimbust_detail = $this->db->get_where('qbg_reimbust_detail', ['id' => $id2])->row_array();

                    if ($reimbust_detail) {
                        $old_image = $reimbust_detail['kwitansi'];
                        if ($old_image != 'default.jpg') {
                            @unlink(FCPATH . './assets/backend/document/reimbust/kwitansi/kwitansi_qbg/' . $old_image);
                        }

                        $this->db->where('id', $id2);
                        $this->db->delete('qbg_reimbust_detail');

                        $kode_deklarasi = $reimbust_detail['deklarasi'];
                        $this->db->update('qbg_deklarasi', ['is_active' => 1], ['kode_deklarasi' => $kode_deklarasi]);
                    }
                }
            }
            ...
            foreach ($pemakaian as $i => $p) {
                ...
                if (isset($_FILES['kwitansi']['name'][$i]) && !empty($_FILES['kwitansi']['name'][$i])) {
                    ...
                    if ($this->upload->do_upload('file')) {
                        ...
                        $reimbust_detail = $this->db->get_where('qbg_reimbust_detail', ['id' => $id])->row_array();
                ...
                if (!empty($kode_prepayment)) {
                    $this->db->update('qbg_prepayment', ['is_active' => 0], ['kode_prepayment' => $kode_prepayment]);
                }
                ...
                if ($kode_prepayment != $kode_prepayment_old && !empty($kode_prepayment_old)) {
                    $this->db->update('qbg_prepayment', ['is_active' => 1], ['kode_prepayment' => $kode_prepayment_old]);
                }

                // Replace data di qbg_reimbust_detail
                $this->db->replace('qbg_reimbust_detail', $data2);

                // mengubah is_active deklarasi awal menjadi 1, dan deklarasi baru menjadi 0
                if (isset($deklarasi_old[$i])) {
                    if ($deklarasi_old[$i]) {
                        $this->db->update('qbg_deklarasi', ['is_active' => 1], ['kode_deklarasi' => $deklarasi_old[$i]]);
                        $this->db->update('qbg_deklarasi', ['is_active' => 0], ['kode_deklarasi' => $deklarasi[$i]]);
                    } else {
                        $this->db->update('qbg_deklarasi', ['is_active' => 0], ['kode_deklarasi' => $deklarasi[$i]]);
                    }
                }
    ...
    public function approve()
    {
        ...
        $this->db->update('qbg_reimbust', $data);
    ...
    public function payment()
    {
        ...
        if ($payment_status === 'paid' && !empty($tgl_pembayaran)) {
            ...
            $reimbust = $this->db->get_where('qbg_reimbust', ['id' => $id])->row_array();

            if ($reimbust && $reimbust['attachment'] && $reimbust['attachment'] != 'default.pdf') {
                @unlink(FCPATH . './assets/backend/document/reimbust/attachment/qbg_attachment/' . $reimbust['attachment']);
                }
        ...
        if (!empty($_FILES['attachment']['name'])) {
            if ($reimbust && !empty($reimbust['attachment']) && $reimbust['attachment'] != 'default.pdf') {
                $oldFile = FCPATH . 'assets/backend/document/reimbust/attachment/qbg_attachment/' . $reimbust['attachment'];
        ...
        // Handle upload file
        if (!empty($_FILES['attachment']['name'])) {
            $config['upload_path'] = 'assets/backend/document/reimbust/attachment/qbg_attachment/';
        ...
        // Update database
        $this->db->where('id', $id);
        $result = $this->db->update('qbg_reimbust', $data);
        if ($result) {
            echo json_encode([
                "status" => TRUE
            ]);
