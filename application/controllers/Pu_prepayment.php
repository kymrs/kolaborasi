...
class Pu_prepayment extends CI_Controller
{
    ...
    public function payment()
    {
        ...
        if ($payment_status === 'paid' && !empty($tgl_pembayaran)) {
            ...
            $reimbust = $this->db->get_where('pu_prepayment', ['id' => $id])->row_array();

            if ($reimbust && $reimbust['attachment'] && $reimbust['attachment'] != 'default.pdf') {
                @unlink(FCPATH . './assets/backend/document/prepayment/attachment/pu_attachment/' . $reimbust['attachment']);
                }
        ...
        if (!empty($_FILES['attachment']['name'])) {
            if ($reimbust && !empty($reimbust['attachment']) && $reimbust['attachment'] != 'default.pdf') {
                $oldFile = FCPATH . 'assets/backend/document/prepayment/attachment/pu_attachment/' . $reimbust['attachment'];
        ...
        // Handle upload file
        if (!empty($_FILES['attachment']['name'])) {
            $config['upload_path'] = 'assets/backend/document/prepayment/attachment/pu_attachment/';
        ...
        // Update database
        $this->db->where('id', $id);
        $result = $this->db->update('pu_prepayment', $data);
        if ($result) {
            echo json_encode([
                "status" => TRUE
            ]);
