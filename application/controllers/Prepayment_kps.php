...
class Prepayment_kps extends CI_Controller
{
    public function __construct()
    {
        parent::__construct();
        $this->load->model('backend/M_prepayment_kps');
    ...
    public function index()
    {
        ...
        $data['title'] = "backend/prepayment_kps/prepayment_list_kps";
    ...
    {
        $this->load->view('backend/prepayment_kps/prepayment_pdf');
    }
    ...
    function get_list()
    {
        ...
        $fullname = $this->db->select('name')
            ...
            ->get()
            ->row('name');
        $list = $this->M_prepayment_kps->get_datatables();
        $data = array();
        $no = $_POST['start'];
        ...
        foreach ($list as $field) {
            ...
            $action_read = ($read == 'Y') ? '<a href="prepayment_kps/read_form/' . $field->id . '" class="btn btn-info btn-circle btn-sm" title="Read"><i class="fa fa-eye"></i></a>&nbsp;' : '';
            $action_edit = ($edit == 'Y') ? '<a href="prepayment_kps/edit_form/' . $field->id . '" class="btn btn-warning btn-circle btn-sm" title="Edit"><i class="fa fa-edit"></i></a>&nbsp;' : '';
            $action_delete = ($delete == 'Y') ? '<a onclick="delete_data(' . "'" . $field->id . "'" . ')" class="btn btn-danger btn-circle btn-sm" title="Delete"><i class="fa fa-trash"></i></a>&nbsp;' : '';
            $action_print = ($print == 'Y') ? '<a class="btn btn-success btn-circle btn-sm" target="_blank" href="prepayment_kps/generate_pdf/' . $field->id . '"><i class="fas fa-file-pdf"></i></a>' : '';
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->M_prepayment_kps->count_all(),
            "recordsFiltered" => $this->M_prepayment_kps->count_filtered(),
            "data" => $data,
        );
    ...
    {
        $data['id'] = $id;
        $data['user'] = $this->M_prepayment_kps->get_by_id($id);
        ...
        $data['app2_name'] = $this->db->select('name')
            ...
            ->get()
            ->row('name');
        $data['title'] = 'backend/prepayment_kps/prepayment_read_kps';
    ...
    public function add_form()
    {
        ...
        $data['id'] = 0;
        $data['title'] = 'backend/prepayment_kps/prepayment_form_kps';
        $data['title_view'] = 'Prepayment Form';
        $data['rek_options'] = $this->M_prepayment_kps->options($id_user)->result_array();
        $this->load->view('backend/home', $data);
    }
    ...
    public function generate_kode()
    {
        ...
        $kode = $this->M_prepayment_kps->max_kode($date)->row();
    ...
    {
        $data['master'] = $this->M_prepayment_kps->get_by_id($id);
        $data['transaksi'] = $this->M_prepayment_kps->get_by_id_detail($id);
    ...
    {
        $data = $this->M_prepayment_kps->get_by_id_detail($id);
        echo json_encode($data);
    }
    ...
    public function add()
    {
        ...
        $approval = $this->M_prepayment_kps->approval($this->session->userdata('id_user'));
        ...
        if ($inserted) {
            ...
            $this->M_prepayment_kps->save_detail($data2);
        }
    ...
    function delete($id)
    {
        $this->M_prepayment_kps->delete($id);
        $this->M_prepayment_kps->delete_detail($id);
        echo json_encode(array("status" => TRUE));
    }
