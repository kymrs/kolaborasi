...
class Sam_prepayment extends CI_Controller
{
    public function __construct()
    {
        parent::__construct();
        $this->load->model('backend/M_sam_prepayment');
    ...
    public function index()
    {
        ...
        $data['title'] = "backend/sam_prepayment/sam_prepayment_list";
    ...
    {
        $this->load->view('backend/sam_prepayment/prepayment_pdf');
    }
    ...
    function get_list()
    {
        ...
        $fullname = $this->db->select('name')
            ...
            ->get()
            ->row('name');
        $list = $this->M_sam_prepayment->get_datatables();
        $data = array();
        $no = $_POST['start'];
        ...
        foreach ($list as $field) {
            ...
            $action_read = ($read == 'Y') ? '<a href="sam_prepayment/read_form/' . $field->id . '" class="btn btn-info btn-circle btn-sm" title="Read"><i class="fa fa-eye"></i></a>&nbsp;' : '';
            $action_edit = ($edit == 'Y') ? '<a href="sam_prepayment/edit_form/' . $field->id . '" class="btn btn-warning btn-circle btn-sm" title="Edit"><i class="fa fa-edit"></i></a>&nbsp;' : '';
            $action_delete = ($delete == 'Y') ? '<a onclick="delete_data(' . "'" . $field->id . "'" . ')" class="btn btn-danger btn-circle btn-sm" title="Delete"><i class="fa fa-trash"></i></a>&nbsp;' : '';
            $action_print = ($print == 'Y') ? '<a class="btn btn-success btn-circle btn-sm" target="_blank" href="sam_prepayment/generate_pdf/' . $field->id . '"><i class="fas fa-file-pdf"></i></a>' : '';
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->M_sam_prepayment->count_all(),
            "recordsFiltered" => $this->M_sam_prepayment->count_filtered(),
            "data" => $data,
        );
    ...
    {
        $data['id'] = $id;
        $data['user'] = $this->M_sam_prepayment->get_by_id($id);
        ...
        $data['app2_name'] = $this->db->select('name')
            ...
            ->get()
            ->row('name');
        $data['title'] = 'backend/sam_prepayment/sam_prepayment_read';
    ...
    public function add_form()
    {
        ...
        $data['id_pembuat'] = 0;
        $data['id'] = 0;
        $data['title'] = 'backend/sam_prepayment/sam_prepayment_form';
        $data['title_view'] = 'Prepayment Form';
        $data['rek_options'] = $this->M_sam_prepayment->options($data['id_user'])->result_array();
    ...
    public function generate_kode()
    {
        ...
        $kode = $this->M_sam_prepayment->max_kode($date)->row();
    ...
    function edit_form($id)
    {
        ...
        $data['id_pembuat'] = $this->M_sam_prepayment->get_by_id($id)->id_user;
    ...
    {
        $data['master'] = $this->M_sam_prepayment->get_by_id($id);
        $data['transaksi'] = $this->M_sam_prepayment->get_by_id_detail($id);
    ...
    {
        $data = $this->M_sam_prepayment->get_by_id_detail($id);
        echo json_encode($data);
    }
    ...
    public function add()
    {
        ...
        if ($valid) {
            $inserted = $this->M_sam_prepayment->save($data);
        }
        ...
        if ($inserted) {
            ...
            $this->M_sam_prepayment->save_detail($data2);
        }
    ...
    public function update()
    {
        ...
        $nominal = $this->input->post('hidden_nominal[]');
        $keterangan = $this->input->post('keterangan[]');
        if ($this->db->update('sam_prepayment', $data)) {
            ...
            if (!empty($deletedRows)) {
                foreach ($deletedRows as $id2) {
                    ...
                    $this->db->delete('sam_prepayment_detail');
                }
            }
            ...
            for ($i = 1; $i <= count($_POST['rincian']); $i++) {
                ...
                $this->db->replace('sam_prepayment_detail', $data2[$i - 1]);
            }
        }
        echo json_encode(array("status" => TRUE));
    }
    ...
    function delete($id)
    {
        $this->M_sam_prepayment->delete($id);
        $this->M_sam_prepayment->delete_detail($id);
        echo json_encode(array("status" => TRUE));
    }
    ...
    public function approve()
    {
        ...
        $this->db->update('sam_prepayment', $data);
    ...
    public function generate_pdf($id)
    {
        ...
        // Output the PDF
        $pdf->Output('I', 'sam_prepayment.pdf');
    }
    ...
    public function payment()
    {
        ...
        if ($payment_status === 'paid' && !empty($tgl_pembayaran)) {
            ...
            $reimbust = $this->db->get_where('sam_prepayment', ['id' => $id])->row_array();

            if ($reimbust && $reimbust['attachment'] && $reimbust['attachment'] != 'default.pdf') {
                @unlink(FCPATH . './assets/backend/document/prepayment/attachment/sam_attachment/' . $reimbust['attachment']);
                }
        ...
        if (!empty($_FILES['attachment']['name'])) {
            if ($reimbust && !empty($reimbust['attachment']) && $reimbust['attachment'] != 'default.pdf') {
                $oldFile = FCPATH . 'assets/backend/document/prepayment/attachment/sam_attachment/' . $reimbust['attachment'];
        ...
        // Handle upload file
        if (!empty($_FILES['attachment']['name'])) {
            $config['upload_path'] = 'assets/backend/document/prepayment/attachment/sam_attachment/';
        ...
        // Update database
        $this->db->where('id', $id);
        $result = $this->db->update('sam_prepayment', $data);
        if ($result) {
            echo json_encode([
                "status" => TRUE
            ]);
