...
class Holding_karyawan extends CI_Controller
{

    function __construct()
    {
        parent::__construct();
        $this->load->model('backend/M_holding_karyawan');
    ...
    public function index()
    {
        ...
        $data['karyawan'] = $this->db->select('npk, nama_lengkap')->get('holding_karyawan')->result_array();
        $data['title'] = "backend/holding_karyawan/holding_karyawan_list";
    ...
    public function e_pkwt()
    {
        ...
        ($akses->view_level == 'N' ? redirect('auth') : '');
        $data['add'] = $akses->add_level;
        $data['nama_pt'] = $this->db
            ->select('nama_pt')
            ->distinct()
            ->where('nama_pt IS NOT NULL')
            ->where('nama_pt !=', '')
            ->order_by('nama_pt', 'ASC')
            ->get('holding_karyawan')
            ->result_array();
        $data['title'] = "backend/holding_karyawan/holding_pkwt_list";
    ...
    function get_list()
    {
        ...
        $list = $this->M_holding_karyawan->get_datatables();
        $data = array();
        $no = $_POST['start'];
        ...
        foreach ($list as $field) {
            ...
            $action_read = ($read == 'Y') ? '<a href="holding_karyawan/read_form/' . $field->id . '" class="btn btn-info btn-circle btn-sm" title="Read"><i class="fa fa-eye"></i></a>&nbsp;' : '';
            $action_edit = ($edit == 'Y') ? '<a href="holding_karyawan/edit_form/' . $field->id . '" class="btn btn-warning btn-circle btn-sm" title="Edit"><i class="fa fa-edit"></i></a>&nbsp;' : '';
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->M_holding_karyawan->count_all(),
            "recordsFiltered" => $this->M_holding_karyawan->count_filtered(),
            "data" => $data,
        );
    ...
    function get_list2()
    {
        ...
        $list = $this->M_holding_karyawan->get_datatables2();
        $data = array();
        $no = $_POST['start'];
        ...
        foreach ($list as $field) {
            ...
            $row[] = $no;
            $row[] = $action;
            $row[] = $unit_bisnis['nama_pt'];
            $row[] = $unit_bisnis['unit_bisnis'];
            ...
            $row[] = !empty($field->created_at_pkwt) ? date('d-m-Y H:i:s', strtotime($field->created_at_pkwt)) : '-';
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->M_holding_karyawan->count_all2(),
            "recordsFiltered" => $this->M_holding_karyawan->count_filtered2(),
            "data" => $data,
        );
        //output dalam format JSON
        echo json_encode($output);
    ...
    function read_form($id)
    {
        $data['id_master'] = $id;
        $data['aksi'] = 'read';
        $data['master'] = $this->M_holding_karyawan->get_by_id($id);
        $data['keluarga'] = $this->db->get_where('holding_keluarga_karyawan', ['npk' => $data['master']->npk])->result();
        $data['kontrak'] = $this->db->get_where('holding_kontrak_pkwt', ['npk' => $data['master']->npk, 'app_status' => 'approved'])->result();
        $data['title_view'] = "Data Karyawan";
        $data['title'] = 'backend/holding_karyawan/holding_karyawan_read';
        $this->load->view('backend/home', $data);
    }
    ...
    function add_form()
    {
        ...
        $data['sub_bisnis_list'] = $this->db->get('holding_sub_bisnis')->result_array();
        $data['title'] = 'backend/holding_karyawan/holding_karyawan_form';
        $this->load->view('backend/home', $data);
    }
    ...
    function add_form_pkwt()
    {
        ...
        $data['karyawan'] = $this->db->select('npk, nama_lengkap, id_user, unit_bisnis')->get('holding_karyawan')->result_array();
        $data['user'] = $this->db->select('id_user, fullname')->get('tbl_user')->result_array();
        $data['sub_bisnis_list'] = $this->db->get('holding_sub_bisnis')->result_array();
        $data['title'] = 'backend/holding_karyawan/holding_pkwt_form';
        $this->load->view('backend/home', $data);
    }
    ...
    //     $data['title'] = 'backend/holding_karyawan/holding_keluarga_form';
    //     $this->load->view('backend/home', $data);
    // }
    ...
    function edit_form($id)
    {
        ...
        $data['id_master'] = $id;
        $data['master'] = $this->M_holding_karyawan->get_by_id($id);
        $data['keluarga'] = $this->db->get_where('holding_keluarga_karyawan', array('npk' => $data['master']->npk))->result();
        $data['kontrak'] = $this->db->get_where('holding_kontrak_pkwt', array('npk' => $data['master']->npk))->result();
        $data['user'] = $this->db->select('id_user, fullname')->get('tbl_user')->result_array();
        $data['sub_bisnis_list'] = $this->db->get('holding_sub_bisnis')->result_array();
    ...
    function edit_form_pkwt($id)
    {
        $data['aksi'] = 'edit';
        $data['id'] = $id;
        $data['karyawan'] = $this->db->select('id_user, npk, nama_lengkap, unit_bisnis')->get('holding_karyawan')->result_array();
        $data['transaksi'] = $this->M_holding_karyawan->get_by_id2($id);
        $data['user'] = $this->db->select('id_user, fullname')->get('tbl_user')->result_array();
        $data['sub_bisnis_list'] = $this->db->get('holding_sub_bisnis')->result_array();
    ...
    {
        $data['master'] = $this->M_holding_karyawan->get_by_id($id);
        $data['keluarga'] = $this->db->get_where('holding_keluarga_karyawan', ['npk' => $data['master']->npk])->result_array();
    ...
    function get_id($id)
    {
        $data = $this->M_holding_karyawan->get_by_id($id);
        echo json_encode($data);
    }
    ...
    public function generate_npk()
    {
        ...
        $row = $this->db->get('holding_karyawan')->row_array();
    ...
    public function generate_no_perjanjian()
    {
        ...
        if (strtotime($jk_awal) === false || strtotime($jk_akhir) === false) {
            echo json_encode(['status' => false, 'message' => 'Tanggal kontrak tidak valid']);
            return;
        }

        if (strtotime($jk_akhir) < strtotime($jk_awal)) {
            echo json_encode(['status' => false, 'message' => 'Tanggal akhir harus sama atau setelah tanggal awal']);
            return;
        }

        // npk bisa datang dalam format npk-iduser -> ambil bagian npk
        $npk_val = is_string($npk) && strpos($npk, '-') !== false ? explode('-', $npk)[0] : $npk;

        // ambil unit_bisnis dari tabel holding_karyawan
        $sb = $this->db->select('unit_bisnis')->where('npk', $npk_val)->get('holding_karyawan')->row();
        $unit = $sb ? $sb->unit_bisnis : null;
        if (empty($unit)) {
            echo json_encode(['status' => false, 'message' => 'Unit bisnis karyawan tidak ditemukan']);
            return;
        }

        $unit_key = strtolower(trim($unit));

        // cari kode sub_bisnis di tabel holding_sub_bisnis
        $normalized_unit = preg_replace('/[^a-z0-9]/', '', $unit_key);
        $holding_sb_rows = $this->db->select('sub_bisnis, kode')->from('holding_sub_bisnis')->get()->result();
        $code = null;

        foreach ($holding_sb_rows as $row) {
            $normalized_row = preg_replace('/[^a-z0-9]/', '', strtolower($row->sub_bisnis));
            if ($normalized_row === $normalized_unit && !empty($row->kode)) {
        ...
        if (empty($code)) {
            ...
            $lookup = preg_replace('/[^a-z0-9]/', '', $unit_key);
            $code = isset($map[$lookup]) ? $map[$lookup] : strtoupper(preg_replace('/\W+/', '', $unit_key));
        }

        // tahun dokumen berdasarkan tanggal awal kontrak
        $tahun = date('Y', strtotime($jk_awal));

        // gunakan model M_Contract_model untuk mengambil nomor urut berikutnya berdasarkan dokumentasi sub-bisnis
        $this->load->model('backend/M_Contract_model');
        $next = $this->M_Contract_model->peek_next_counter($code, $tahun);
        if ($next === false) {
            echo json_encode(['status' => false, 'message' => 'Gagal generate counter']);
            return;
        }

        $urut = str_pad($next, 3, '0', STR_PAD_LEFT);
        $perusahaan = 'PKWT-' . $code;

        // Kontrak ke berapa untuk karyawan ini
        $this->db->where('npk', $npk_val);
        $kontrak_ke = $this->db->count_all_results('holding_kontrak_pkwt') + 1;
    ...
    public function add_data_karyawan()
    {
        ...
        $this->M_holding_karyawan->save($data);
        ...
        if (is_array($nama_anggota)) {
            ...
            if (count($data_keluarga) > 0) {
                $this->db->insert_batch('holding_keluarga_karyawan', $data_keluarga);
            }
        }
    ...
    public function add_kontrak_karyawan()
    {
        ...
        if (empty($no_perjanjian)) {
            echo json_encode(['status' => false, 'message' => 'No perjanjian harus diisi terlebih dahulu.']);
            return;
        }
        ...
        $this->db->insert('holding_kontrak_pkwt', $data);
        echo json_encode(array("status" => TRUE));
    }
    ...
    public function update()
    {
        ...
        $karyawan_lama = $this->db->select('foto, kk, ktp, npwp, ijazah')->where('id', $id_master)->get('holding_karyawan')->row_array();
        ...
        $this->db->update('holding_karyawan', $data);
        ...
        if (is_array($id_keluarga) && count($id_keluarga) > 0) {
            ...
            $this->db->update_batch('holding_keluarga_karyawan', $data2, 'id');
        }
    ...
    public function update_kontrak_karyawan($id)
    {
        ...
        $this->db->where('id', $id);
        $this->db->update('holding_kontrak_pkwt', $data);
    ...
    function delete_data_karyawan($id)
    {
        ...
        $karyawan = $this->db->select('npk, foto')->where('id', $id)->get('holding_karyawan')->row_array();
        ...
        $this->db->delete('holding_keluarga_karyawan', ['npk' => $karyawan['npk']]);
        $this->db->delete('holding_kontrak_pkwt', ['npk' => $karyawan['npk']]);

        // Hapus data karyawan
        $this->M_holding_karyawan->delete($id);
        echo json_encode(array("status" => TRUE));
    }
    ...
    public function delete_data_pkwt($id)
    {
        ...
        $deleted = $this->M_holding_karyawan->delete_kontrak_by_id($id);
    ...
    public function delete_keluarga()
    {
        ...
        $deleted = $this->M_holding_karyawan->delete_keluarga_by_id($id);
    ...
    public function approve_pkwt()
    {
        ...
        $row = $this->db->get_where('holding_kontrak_pkwt', ['id' => $id])->row();
        if (!$row) {
        ...
        $this->db->where('id', $id)->update('holding_kontrak_pkwt', $update);
    ...
    public function get_expiring_pkwt()
    {
        ...
        $rows = $this->M_holding_karyawan->get_expiring($days);
