...
class Prepayment_ctz extends CI_Controller
{
    public function __construct()
    {
        parent::__construct();
        $this->load->model('backend/M_prepayment_ctz');
    ...
    public function index()
    {
        ...
        $data['title'] = "backend/prepayment_ctz/prepayment_list_ctz";
    ...
    {
        $this->load->view('backend/prepayment_ctz/prepayment_pdf');
    }
    ...
    function get_list()
    {
        ...
        $fullname = $this->db->select('name')
            ...
            ->get()
            ->row('name');
        $list = $this->M_prepayment_ctz->get_datatables();
        $data = array();
        $no = $_POST['start'];
        ...
        foreach ($list as $field) {
            ...
            $action_read = ($read == 'Y') ? '<a href="prepayment_ctz/read_form/' . $field->id . '" class="btn btn-info btn-circle btn-sm" title="Read"><i class="fa fa-eye"></i></a>&nbsp;' : '';
            $action_edit = ($edit == 'Y') ? '<a href="prepayment_ctz/edit_form/' . $field->id . '" class="btn btn-warning btn-circle btn-sm" title="Edit"><i class="fa fa-edit"></i></a>&nbsp;' : '';
            $action_delete = ($delete == 'Y') ? '<a onclick="delete_data(' . "'" . $field->id . "'" . ')" class="btn btn-danger btn-circle btn-sm" title="Delete"><i class="fa fa-trash"></i></a>&nbsp;' : '';
            $action_print = ($print == 'Y') ? '<a class="btn btn-success btn-circle btn-sm" target="_blank" href="prepayment_ctz/generate_pdf/' . $field->id . '"><i class="fas fa-file-pdf"></i></a>' : '';
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->M_prepayment_ctz->count_all(),
            "recordsFiltered" => $this->M_prepayment_ctz->count_filtered(),
            "data" => $data,
        );
    ...
    public function read_form($id)
    {
        $data['notif'] = $this->M_notifikasi->pending_notification();
        $data['id'] = $id;
        $data['user'] = $this->M_prepayment_ctz->get_by_id($id);
        ...
        $data['app2_name'] = $this->db->select('name')
            ...
            ->get()
            ->row('name');
        $data['title'] = 'backend/prepayment_ctz/prepayment_read_ctz';
    ...
    public function add_form()
    {
        ...
        $data['id'] = 0;
        $data['title'] = 'backend/prepayment_ctz/prepayment_form_ctz';
        $data['title_view'] = 'Prepayment Form';
        $data['rek_options'] = $this->M_prepayment_ctz->options($id_user)->result_array();
        $data['notif'] = $this->M_notifikasi->pending_notification();
        $this->load->view('backend/home', $data);
    }
    ...
    public function generate_kode()
    {
        ...
        $kode = $this->M_prepayment_ctz->max_kode($date)->row();
    ...
    {
        $data['master'] = $this->M_prepayment_ctz->get_by_id($id);
        $data['transaksi'] = $this->M_prepayment_ctz->get_by_id_detail($id);
    ...
    {
        $data = $this->M_prepayment_ctz->get_by_id_detail($id);
        echo json_encode($data);
    }
    ...
    public function add()
    {
        ...
        $approval = $this->M_prepayment_ctz->approval($this->session->userdata('id_user'));
        ...
        $inserted = $this->M_prepayment_ctz->save($data);
        ...
        if ($inserted) {
            ...
            $this->M_prepayment_ctz->save_detail($data2);
        }
    ...
    public function update()
    {
        ...
        $nominal = $this->input->post('hidden_nominal[]');
        $keterangan = $this->input->post('keterangan[]');
        if ($this->db->update('ctz_prepayment', $data)) {
    ...
    function delete($id)
    {
        $this->M_prepayment_ctz->delete($id);
        $this->M_prepayment_ctz->delete_detail($id);
        echo json_encode(array("status" => TRUE));
    }
    ...
    public function generate_pdf($id)
    {
        ...
        // Output the PDF
        $pdf->Output('I', 'Prepayment_CTZ.pdf');
    }
