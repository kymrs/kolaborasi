...
class Kps_prepayment extends CI_Controller
{
    public function __construct()
    {
        parent::__construct();
        $this->load->model('backend/M_kps_prepayment');
    ...
    public function index()
    {
        ...
        $data['title'] = "backend/kps_prepayment/kps_prepayment_list";
    ...
    {
        $this->load->view('backend/kps_prepayment/prepayment_pdf');
    }
    ...
    function get_list()
    {
        ...
        $fullname = $this->db->select('name')
            ...
            ->get()
            ->row('name');
        $list = $this->M_kps_prepayment->get_datatables();
        $data = array();
        $no = $_POST['start'];
        ...
        foreach ($list as $field) {
            ...
            $action_read = ($read == 'Y') ? '<a href="kps_prepayment/read_form/' . $field->id . '" class="btn btn-info btn-circle btn-sm" title="Read"><i class="fa fa-eye"></i></a>&nbsp;' : '';
            $action_edit = ($edit == 'Y') ? '<a href="kps_prepayment/edit_form/' . $field->id . '" class="btn btn-warning btn-circle btn-sm" title="Edit"><i class="fa fa-edit"></i></a>&nbsp;' : '';
            $action_delete = ($delete == 'Y') ? '<a onclick="delete_data(' . "'" . $field->id . "'" . ')" class="btn btn-danger btn-circle btn-sm" title="Delete"><i class="fa fa-trash"></i></a>&nbsp;' : '';
            $action_print = ($print == 'Y') ? '<a class="btn btn-success btn-circle btn-sm" target="_blank" href="kps_prepayment/generate_pdf/' . $field->id . '"><i class="fas fa-file-pdf"></i></a>' : '';
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->M_kps_prepayment->count_all(),
            "recordsFiltered" => $this->M_kps_prepayment->count_filtered(),
            "data" => $data,
        );
    ...
    {
        $data['id'] = $id;
        $data['user'] = $this->M_kps_prepayment->get_by_id($id);
        ...
        $data['app2_name'] = $this->db->select('name')
            ...
            ->get()
            ->row('name');
        $data['title'] = 'backend/kps_prepayment/kps_prepayment_read';
    ...
    public function add_form()
    {
        ...
        $data['id'] = 0;
        $data['title'] = 'backend/kps_prepayment/kps_prepayment_form';
        $data['title_view'] = 'Prepayment Form';
        $data['rek_options'] = $this->M_kps_prepayment->options($data['id_user'])->result_array();
        $this->load->view('backend/home', $data);
    }
    ...
    public function generate_kode()
    {
        ...
        $kode = $this->M_kps_prepayment->max_kode($date)->row();
    ...
    function edit_form($id)
    {
        ...
        $data['id_pembuat'] = $this->M_kps_prepayment->get_by_id($id)->id_user;
    ...
    {
        $data['master'] = $this->M_kps_prepayment->get_by_id($id);
        $data['transaksi'] = $this->M_kps_prepayment->get_by_id_detail($id);
    ...
    {
        $data = $this->M_kps_prepayment->get_by_id_detail($id);
        echo json_encode($data);
    }
    ...
    public function add()
    {
        ...
        if ($valid) {
            $inserted = $this->M_kps_prepayment->save($data);
        }
        ...
        if ($inserted) {
            ...
            $this->M_kps_prepayment->save_detail($data2);
        }
    ...
    public function update()
    {
        ...
        $nominal = $this->input->post('hidden_nominal[]');
        $keterangan = $this->input->post('keterangan[]');
        if ($this->db->update('kps_prepayment', $data)) {
            ...
            if (!empty($deletedRows)) {
                foreach ($deletedRows as $id2) {
                    ...
                    $this->db->delete('kps_prepayment_detail');
                }
            }
            ...
            for ($i = 1; $i <= count($_POST['rincian']); $i++) {
                ...
                $this->db->replace('kps_prepayment_detail', $data2[$i - 1]);
            }
        }
        echo json_encode(array("status" => TRUE));
    }
    ...
    function delete($id)
    {
        $this->M_kps_prepayment->delete($id);
        $this->M_kps_prepayment->delete_detail($id);
        echo json_encode(array("status" => TRUE));
    }
    ...
    public function approve()
    {
        ...
        $this->db->update('kps_prepayment', $data);
    ...
    public function generate_pdf($id)
    {
        ...
        // Output the PDF
        $pdf->Output('I', 'kps_prepayment.pdf');
    }
