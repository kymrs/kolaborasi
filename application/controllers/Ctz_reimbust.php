...
class Ctz_reimbust extends CI_Controller
{
    public function __construct()
    {
        parent::__construct();
        $this->load->model('backend/M_ctz_reimbust');
    ...
    function get_list()
    {
        ...
        $fullname = $this->db->select('name')
            ...
            ->get()
            ->row('name');
        $list = $this->M_ctz_reimbust->get_datatables();
        $data = array();
        $no = $_POST['start'];
        ...
        foreach ($list as $field) {
            ...
            $action_read = ($read == 'Y') ? '<a href="ctz_reimbust/read_form/' . $field->id . '" class="btn btn-info btn-circle btn-sm" title="Read"><i class="fa fa-eye"></i></a>&nbsp;' : '';
            $action_edit = ($edit == 'Y') ? '<a href="ctz_reimbust/edit_form/' . $field->id . '" class="btn btn-warning btn-circle btn-sm" title="Edit"><i class="fa fa-edit"></i></a>&nbsp;' : '';
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->M_ctz_reimbust->count_all(),
            "recordsFiltered" => $this->M_ctz_reimbust->count_filtered(),
            "data" => $data,
        );
    ...
    function get_list2()
    {
        ...
        $fullname = $this->db->select('name')
            ...
            ->get()
            ->row('name');
        $list = $this->M_ctz_reimbust->get_datatables2();
        $data = array();
        $no = $_POST['start'];
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->M_ctz_reimbust->count_all2(),
            "recordsFiltered" => $this->M_ctz_reimbust->count_filtered2(),
            "data" => $data,
        );
        //output dalam format JSON
        echo json_encode($output);
    ...
    function get_list3()
    {
        ...
        $fullname = $this->db->select('name')
            ...
            ->get()
            ->row('name');
        $list = $this->M_ctz_reimbust->get_datatables3();
        $data = array();
        $no = $_POST['start'];
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->M_ctz_reimbust->count_all3(),
            "recordsFiltered" => $this->M_ctz_reimbust->count_filtered3(),
            "data" => $data,
        );
        //output dalam format JSON
        echo json_encode($output);
    ...
    function read_form($id)
    {
        ...
        $data['title'] = 'backend/ctz_reimbust/ctz_reimbust_read';
    ...
    public function generate_pdf($id)
    {
        ...
        $data['master'] = $this->M_ctz_reimbust->get_by_id($id);
        $data['transaksi'] = $this->M_ctz_reimbust->get_by_id_detail($id);
        ...
        // Output the PDF
        $pdf->Output('I', 'ctz_reimbust - ' . $data['master']->kode_reimbust . '.pdf');
    }
    ...
    public function generate_kode()
    {
        ...
        $kode = $this->M_ctz_reimbust->max_kode($date)->row();
    ...
    {
        $data = $this->M_ctz_reimbust->get_by_id_detail($id);
        echo json_encode($data);
    }
    ...
    public function detail_deklarasi()
    {
        if ($this->input->is_ajax_request()) {
            ...
            if ($deklarasiRecord) {
                ...
                $redirect_url = site_url('ctz_datadeklarasi/read_form/' . $deklarasiId);
    ...
    public function add()
    {
        ...
        if ($valid) {
            $reimbust_id = $this->M_ctz_reimbust->save($data1);
        }
