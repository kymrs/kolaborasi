...
class Reimbust_mc extends CI_Controller
{
    public function __construct()
    {
        parent::__construct();
        $this->load->model('backend/M_reimbust_mc');
    ...
    public function index()
    {
        ...
        $data['title'] = "backend/reimbust_mc/reimbust_list_mc";
    ...
    function get_list()
    {
        ...
        $fullname = $this->db->select('name')
            ...
            ->get()
            ->row('name');
        $list = $this->M_reimbust_mc->get_datatables();
        $data = array();
        $no = $_POST['start'];
        ...
        foreach ($list as $field) {
            ...
            $action_read = ($read == 'Y') ? '<a href="reimbust_mc/read_form/' . $field->id . '" class="btn btn-info btn-circle btn-sm" title="Read"><i class="fa fa-eye"></i></a>&nbsp;' : '';
            $action_edit = ($edit == 'Y') ? '<a href="reimbust_mc/edit_form/' . $field->id . '" class="btn btn-warning btn-circle btn-sm" title="Edit"><i class="fa fa-edit"></i></a>&nbsp;' : '';
            $action_delete = ($delete == 'Y') ? '<a onclick="delete_data(' . "'" . $field->id . "'" . ')" class="btn btn-danger btn-circle btn-sm" title="Delete"><i class="fa fa-trash"></i></a>&nbsp;' : '';
            $action_print = ($print == 'Y') ? '<a class="btn btn-success btn-circle btn-sm" target="_blank" href="reimbust_mc/generate_pdf/' . $field->id . '"><i class="fas fa-file-pdf"></i></a>' : '';
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->M_reimbust_mc->count_all(),
            "recordsFiltered" => $this->M_reimbust_mc->count_filtered(),
            "data" => $data,
        );
    ...
    function get_list2()
    {
        ...
        $fullname = $this->db->select('name')
            ...
            ->get()
            ->row('name');
        $list = $this->M_reimbust_mc->get_datatables2();
        $data = array();
        $no = $_POST['start'];
        ...
        foreach ($list as $field) {
            ...
            if ($field->app_name == $fullname) {
                $action = '<a href="datadeklarasi_mc/read_form/' . $field->id . '" class="btn btn-info btn-circle btn-sm" title="Read"><i class="fa fa-eye"></i></a>
                                <a class="btn btn-success btn-circle btn-sm" href="datadeklarasi_mc/generate_pdf/' . $field->id . '"><i class="fas fa-file-pdf"></i></a>';
                ...
                $action = '<a href="datadeklarasi_mc/read_form/' . $field->id . '" class="btn btn-info btn-circle btn-sm" title="Read"><i class="fa fa-eye"></i></a>
                    <a href="datadeklarasi_mc/edit_form/' . $field->id . '" class="btn btn-warning btn-circle btn-sm" title="Edit"><i class="fa fa-edit"></i></a>
                    <a class="btn btn-success btn-circle btn-sm" href="datadeklarasi_mc/generate_pdf/' . $field->id . '"><i class="fas fa-file-pdf"></i></a>';
            } elseif ($field->app_status == 'approved') {
                $action = '<a href="datadeklarasi_mc/read_form/' . $field->id . '" class="btn btn-info btn-circle btn-sm" title="Read"><i class="fa fa-eye"></i></a>
                            <a class="btn btn-success btn-circle btn-sm" href="datadeklarasi_mc/generate_pdf/' . $field->id . '"><i class="fas fa-file-pdf"></i></a>';
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->M_reimbust_mc->count_all2(),
            "recordsFiltered" => $this->M_reimbust_mc->count_filtered2(),
            "data" => $data,
        );
        //output dalam format JSON
        echo json_encode($output);
    ...
    function get_list3()
    {
        ...
        $fullname = $this->db->select('name')
            ...
            ->get()
            ->row('name');
        $list = $this->M_reimbust_mc->get_datatables3();
        $data = array();
        $no = $_POST['start'];
        ...
        foreach ($list as $field) {
            ...
            // MENENTUKAN ACTION APA YANG AKAN DITAMPILKAN DI LIST DATA TABLES
            if ($field->app_name == $fullname) {
                $action = '<a href="prepayment_mc/read_form/' . $field->id . '" class="btn btn-info btn-circle btn-sm" title="Read"><i class="fa fa-eye"></i></a>
                                <a class="btn btn-success btn-circle btn-sm" href="prepayment_mc/generate_pdf/' . $field->id . '"><i class="fas fa-file-pdf"></i></a>';
            } elseif ($field->app2_name == $fullname) {
                $action = '<a href="prepayment_mc/read_form/' . $field->id . '" class="btn btn-info btn-circle btn-sm" title="Read"><i class="fa fa-eye"></i></a>     
                                <a class="btn btn-success btn-circle btn-sm" href="prepayment_mc/generate_pdf/' . $field->id . '"><i class="fas fa-file-pdf"></i></a>';
            } elseif (in_array($field->status, ['rejected', 'approved'])) {
                ...
                $action = '<a href="prepayment_mc/read_form/' . $field->id . '" class="btn btn-info btn-circle btn-sm" title="Read"><i class="fa fa-eye"></i></a>
                    <a href="prepayment_mc/edit_form/' . $field->id . '" class="btn btn-warning btn-circle btn-sm" title="Edit"><i class="fa fa-edit"></i></a>
                    <a class="btn btn-success btn-circle btn-sm" href="prepayment_mc/generate_pdf/' . $field->id . '"><i class="fas fa-file-pdf"></i></a>';
            } elseif ($field->app_status == 'approved') {
                $action = '<a href="prepayment_mc/read_form/' . $field->id . '" class="btn btn-info btn-circle btn-sm" title="Read"><i class="fa fa-eye"></i></a>
                            <a class="btn btn-success btn-circle btn-sm" href="prepayment_mc/generate_pdf/' . $field->id . '"><i class="fas fa-file-pdf"></i></a>';
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->M_reimbust_mc->count_all3(),
            "recordsFiltered" => $this->M_reimbust_mc->count_filtered3(),
            "data" => $data,
        );
        //output dalam format JSON
        echo json_encode($output);
    ...
    function read_form($id)
    {
        $data['notif'] = $this->M_notifikasi->pending_notification();
        $data['aksi'] = 'read';
        $data['user'] = $this->M_reimbust_mc->get_by_id($id);
        ...
        $data['title'] = 'backend/reimbust_mc/reimbust_read_mc';
        $this->db->select('kwitansi');
        $this->db->where('reimbust_id', $id);
        $data['kwitansi'] = $this->db->get('mc_reimbust_detail')->result_array();
        $this->load->view('backend/home', $data);
    }
    ...
    public function add_form()
    {
        ...
        $data['id'] = 0;
        $data['aksi'] = 'add';
        $data['rek_options'] = $this->M_reimbust_mc->options()->result_array();
        $data['title_view'] = "Reimbust Form";
        $data['title'] = 'backend/reimbust_mc/reimbust_form_mc';
        $this->load->view('backend/home', $data);
    }
    ...
    public function generate_pdf($id)
    {
        ...
        $data['master'] = $this->M_reimbust_mc->get_by_id($id);
        $data['transaksi'] = $this->M_reimbust_mc->get_by_id_detail($id);
        ...
        // Add total and remaining prepayment_mc
        $pdf->SetFont('Poppins-Bold', '', 10);
        ...
        // Output the PDF
        $pdf->Output('I', 'reimbust_mc - ' . $data['master']->kode_reimbust . '.pdf');
    }
    ...
    public function generate_kode()
    {
        ...
        $kode = $this->M_reimbust_mc->max_kode($date)->row();
    ...
    {
        $data = $this->M_reimbust_mc->get_by_id_detail($id);
        echo json_encode($data);
    }
    ...
    public function detail_deklarasi()
    {
        if ($this->input->is_ajax_request()) {
            ...
            $deklarasiRecord = $this->db->get_where('mc_deklarasi', ['kode_deklarasi' => $deklarasi])->row_array();
            ...
            if ($deklarasiRecord) {
                ...
                $redirect_url = site_url('datadeklarasi_mc/read_form/' . $deklarasiId);
    ...
    public function add()
    {
        ...
        $approval = $this->M_reimbust_mc->approval($this->session->userdata('id_user'));
        ...
        if ($valid) {
            $reimbust_id = $this->M_reimbust_mc->save($data1);
        }
        ...
        for ($i = 1; $i <= count($pemakaian); $i++) {
            ...
            if (!empty($_FILES['kwitansi']['name'][$i])) {
                ...
                $config['upload_path'] = './assets/backend/document/reimbust/kwitansi_mc/';
            ...
            $this->db->update('mc_deklarasi');
        }
        $this->db->set('is_active', 0);
        $this->db->where('kode_prepayment', $this->input->post('kode_prepayment'));
        $this->db->update('mc_prepayment');

        $this->M_reimbust_mc->save_detail($data2);
    ...
    public function update()
    {
        ...
        if ($this->db->update('mc_reimbust', $data)) {
            // 1. Hapus Baris yang Telah Dihapus
            $deletedRows = json_decode($this->input->post('deleted_rows'), true);
            if (!empty($deletedRows)) {
                foreach ($deletedRows as $id2) {
                    $reimbust_detail = $this->db->get_where('mc_reimbust_detail', ['id' => $id2])->row_array();

                    if ($reimbust_detail) {
                        $old_image = $reimbust_detail['kwitansi'];
                        if ($old_image != 'default.jpg') {
                            @unlink(FCPATH . './assets/backend/document/reimbust/kwitansi_mc/' . $old_image);
                        }

                        $this->db->where('id', $id2);
                        $this->db->delete('mc_reimbust_detail');

                        $kode_deklarasi = $reimbust_detail['deklarasi'];
                        $this->db->update('mc_deklarasi', ['is_active' => 1], ['kode_deklarasi' => $kode_deklarasi]);
                    }
                }
            }
            ...
            foreach ($pemakaian as $i => $p) {
                ...
                if (isset($_FILES['kwitansi']['name'][$i]) && !empty($_FILES['kwitansi']['name'][$i])) {
                    ...
                    if ($this->upload->do_upload('file')) {
                        ...
                        $reimbust_detail = $this->db->get_where('mc_reimbust_detail', ['id' => $id])->row_array();
                ...
                // Mengubah data prepayment_mc is_active menjadi 0 pada data prepayment_mc terbaru, jika kode_prepayment ada
                $kode_prepayment = $this->input->post('kode_prepayment');
                if (!empty($kode_prepayment)) {
                    $this->db->update('mc_prepayment', ['is_active' => 0], ['kode_prepayment' => $kode_prepayment]);
                }

                // Mengubah data prepayment_mc is_active menjadi 1 pada data prepayment_mc terlama, jika kode_prepayment_old ada
                $kode_prepayment_old = $this->input->post('kode_prepayment_old');
                if ($kode_prepayment != $kode_prepayment_old && !empty($kode_prepayment_old)) {
                    $this->db->update('mc_prepayment', ['is_active' => 1], ['kode_prepayment' => $kode_prepayment_old]);
                }

                // Replace data di tbl_reimbust_detail
                $this->db->replace('mc_reimbust_detail', $data2);

                // mengubah is_active deklarasi awal menjadi 1, dan deklarasi baru menjadi 0
                if ($deklarasi_old[$i]) {
                    $this->db->update('mc_deklarasi', ['is_active' => 1], ['kode_deklarasi' => $deklarasi_old[$i]]);
                    $this->db->update('mc_deklarasi', ['is_active' => 0], ['kode_deklarasi' => $deklarasi[$i]]);
                } else {
                    $this->db->update('mc_deklarasi', ['is_active' => 0], ['kode_deklarasi' => $deklarasi[$i]]);
                }
            }
    ...
    public function approve()
    {
        ...
        $this->db->update('mc_reimbust', $data);
    ...
    {
        $this->db->where('id', $this->input->post('id'));
        $this->db->update('mc_reimbust', ['payment_status' => $this->input->post('payment_status')]);
