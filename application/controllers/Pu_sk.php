...
class Pu_sk extends CI_Controller
{

    function __construct()
    {
        parent::__construct();
        $this->load->model('backend/M_pu_sk');
    ...
    {
        $data['title'] = "backend/pu_sk/pu_sk_list";
    ...
    function get_list()
    {
        ...
        $list = $this->M_pu_sk->get_datatables($status);
        $data = array();
        $no = $_POST['start'];
        ...
        foreach ($list as $field) {
            ...
            $row[] = $action;
            $row[] = $field->nama_file;
            $row[] = '<a href="' . base_url('assets/backend/document/sk/pu_sk/' . $field->file) . '" target="_blank">' . 'Lihat File' . '</a>';
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->M_pu_sk->count_all(),
            "recordsFiltered" => $this->M_pu_sk->count_filtered(),
            "data" => $data,
        );
    ...
    function get_id($id)
    {
        $data = $this->M_pu_sk->get_by_id($id);
    ...
    public function add()
    {
        ...
        $upload_path = FCPATH . 'assets/backend/document/sk/pu_sk/';
    ...
    public function update()
    {
        ...
        $old_data  = $this->db->get_where('pu_sk', array('id' => $id))->row();
        $upload_path = FCPATH . 'assets/backend/document/sk/pu_sk/';
    ...
    public function delete($id)
    {
        ...
        $row = $this->db->get_where('pu_sk', array('id' => $id))->row();
