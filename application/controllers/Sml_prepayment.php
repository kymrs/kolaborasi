...
class Sml_prepayment extends CI_Controller
{
    ...
    function get_list()
    {
        ...
        $fullname = $this->db->select('name')
            ...
            ->get()
            ->row('name');
        $list = $this->M_sml_prepayment->get_datatables();
        $data = array();
        $no = $_POST['start'];
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->M_sml_prepayment->count_all(),
            "recordsFiltered" => $this->M_sml_prepayment->count_filtered(),
            "data" => $data,
        );
    ...
    public function read_form($id)
    {
        ...
        $data['app2_name'] = $this->db->select('name')
            ...
            ->get()
            ->row('name');
        $data['title'] = 'backend/sml_prepayment/sml_prepayment_read';
    ...
    public function generate_kode()
    {
        ...
        $kode = $this->M_sml_prepayment->max_kode($date)->row();
    ...
    {
        $data['master'] = $this->M_sml_prepayment->get_by_id($id);
        $data['transaksi'] = $this->M_sml_prepayment->get_by_id_detail($id);
    ...
    {
        $data = $this->M_sml_prepayment->get_by_id_detail($id);
        echo json_encode($data);
    }
    ...
    public function add()
    {
        ...
        if ($inserted) {
            ...
            $this->M_sml_prepayment->save_detail($data2);
        }
        ...
        if ($this->db->update('tbl_prepayment', $data)) {
            ...
            if (!empty($deletedRows)) {
                foreach ($deletedRows as $id2) {
                    ...
                    $this->db->delete('sml_prepayment_detail');
                }
            }
            ...
            for ($i = 1; $i <= count($_POST['rincian']); $i++) {
                ...
                $this->db->replace('sml_prepayment_detail', $data2[$i - 1]);
            }
        }
        echo json_encode(array("status" => TRUE));
    }
    ...
    function delete($id)
    {
        $this->M_sml_prepayment->delete($id);
        $this->M_sml_prepayment->delete_detail($id);
        echo json_encode(array("status" => TRUE));
    }
