...
class Datadeklarasi_sw extends CI_Controller
{

    function __construct()
    {
        parent::__construct();
        $this->load->model('backend/M_datadeklarasi_sw');
    ...
    public function index()
    {
        ...
        $data['title'] = "backend/datadeklarasi_sw/deklarasi_list_sw";
    ...
    function get_list()
    {
        ...
        $fullname = $this->db->select('name')
            ...
            ->get()
            ->row('name');
        $list = $this->M_datadeklarasi_sw->get_datatables();
        $data = array();
        $no = $_POST['start'];
        ...
        foreach ($list as $field) {
            ...
            $action_read = ($read == 'Y') ? '<a href="datadeklarasi_sw/read_form/' . $field->id . '" class="btn btn-info btn-circle btn-sm" title="Read"><i class="fa fa-eye"></i></a>&nbsp;' : '';
            $action_edit = ($edit == 'Y') ? '<a href="datadeklarasi_sw/edit_form/' . $field->id . '" class="btn btn-warning btn-circle btn-sm" title="Edit"><i class="fa fa-edit"></i></a>&nbsp;' : '';
            $action_delete = ($delete == 'Y') ? '<a onclick="delete_data(' . "'" . $field->id . "'" . ')" class="btn btn-danger btn-circle btn-sm" title="Delete"><i class="fa fa-trash"></i></a>&nbsp;' : '';
            $action_print = ($print == 'Y') ? '<a class="btn btn-success btn-circle btn-sm" target="_blank" href="datadeklarasi_sw/generate_pdf/' . $field->id . '"><i class="fas fa-file-pdf"></i></a>' : '';
            ...
            if ($field->app_name == $fullname && $field->id_pengaju != $this->session->userdata('id_user')) {
                ...
                $action = $action_read . $action_print;
            } elseif ($field->id_pengaju != $this->session->userdata('id_user') && $field->app4_name == $fullname) {
                $action = $action_read . $action_print;
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->M_datadeklarasi_sw->count_all(),
            "recordsFiltered" => $this->M_datadeklarasi_sw->count_filtered(),
            "data" => $data,
        );
    ...
    function read_form($id)
    {
        $data['notif'] = $this->M_notifikasi->pending_notification();
        $data['id'] = $id;
        $data['user'] = $this->M_datadeklarasi_sw->get_by_id($id);
        ...
        $data['title'] = 'backend/datadeklarasi_sw/deklarasi_read_sw';
        $this->load->view('backend/home', $data);
    }
    ...
    function add_form()
    {
        ...
        $data['aksi'] = 'update';
        $data['title'] = 'backend/datadeklarasi_sw/deklarasi_form_sw';
        $this->load->view('backend/home', $data);
    }
    ...
    {
        $data['master'] = $this->M_datadeklarasi_sw->get_by_id($id);
    ...
    public function generate_kode()
    {
        ...
        $kode = $this->M_datadeklarasi_sw->max_kode($date)->row();
    ...
    public function add()
    {
        ...
        $approval = $this->M_datadeklarasi_sw->approval($this->session->userdata('id_user'));
    ...
    function delete($id)
    {
        $this->M_datadeklarasi_sw->delete($id);
        echo json_encode(array("status" => TRUE));
    }
