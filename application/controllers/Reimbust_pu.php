...
class Reimbust_pu extends CI_Controller
{
    ...
    {
        parent::__construct();
        $this->load->model('backend/M_reimbust_pu');
    ...
    function get_list()
    {
        ...
        $fullname = $this->db->select('name')
            ...
            ->get()
            ->row('name');
        $list = $this->M_reimbust_pu->get_datatables();
        $data = array();
        $no = $_POST['start'];
        ...
        foreach ($list as $field) {
            ...
            $action_read = ($read == 'Y') ? '<a href="reimbust_pu/read_form/' . $field->id . '" class="btn btn-info btn-circle btn-sm" title="Read"><i class="fa fa-eye"></i></a>&nbsp;' : '';
            $action_edit = ($edit == 'Y') ? '<a href="reimbust_pu/edit_form/' . $field->id . '" class="btn btn-warning btn-circle btn-sm" title="Edit"><i class="fa fa-edit"></i></a>&nbsp;' : '';
            $action_delete = ($delete == 'Y') ? '<a onclick="delete_data(' . "'" . $field->id . "'" . ')" class="btn btn-danger btn-circle btn-sm" title="Delete"><i class="fa fa-trash"></i></a>&nbsp;' : '';
            $action_upload = ($upload == 'Y') ? '<a class="btn btn-success btn-circle btn-sm" href="reimbust_pu/generate_pdf/' . $field->id . '"><i class="fas fa-file-pdf"></i></a>' : '';
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->M_reimbust_pu->count_all(),
            "recordsFiltered" => $this->M_reimbust_pu->count_filtered(),
            "data" => $data,
        );
    ...
    function get_list2()
    {
        ...
        $fullname = $this->db->select('name')
            ...
            ->get()
            ->row('name');
        $list = $this->M_reimbust_pu->get_datatables2();
        $data = array();
        $no = $_POST['start'];
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->M_reimbust_pu->count_all2(),
            "recordsFiltered" => $this->M_reimbust_pu->count_filtered2(),
            "data" => $data,
        );
        //output dalam format JSON
        echo json_encode($output);
    ...
    function get_list3()
    {
        ...
        $fullname = $this->db->select('name')
            ...
            ->get()
            ->row('name');
        $list = $this->M_reimbust_pu->get_datatables3();
        $data = array();
        $no = $_POST['start'];
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->M_reimbust_pu->count_all3(),
            "recordsFiltered" => $this->M_reimbust_pu->count_filtered3(),
            "data" => $data,
        );
        //output dalam format JSON
        echo json_encode($output);
    ...
    public function generate_pdf($id)
    {
        ...
        $data['transaksi'] = $this->M_reimbust_pu->get_by_id_detail($id);
        ...
        // Logo
        $pdf->Image(base_url('') . '/assets/backend/img/pengenumroh.png', 10, -3, 45, 45);
    ...
    public function generate_kode()
    {
        ...
        $kode = $this->M_reimbust_pu->max_kode($date)->row();
        if (empty($kode->kode_reimbust)) {
            $no_urut = 1;
        } else {
            $bln = substr($kode->kode_reimbust, 3, 2);
    ...
    {
        $data = $this->M_reimbust_pu->get_by_id_detail($id);
        echo json_encode($data);
    }
    ...
    public function add()
    {
        ...
        $reimbust_id = $this->M_reimbust_pu->save($data1);
