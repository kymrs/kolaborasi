...
class Sam_datadeklarasi extends CI_Controller
{

    function __construct()
    {
        parent::__construct();
        $this->load->model('backend/M_sam_datadeklarasi');
    ...
    public function index()
    {
        ...
        $data['title'] = "backend/sam_datadeklarasi/sam_deklarasi_list";
    ...
    function get_list()
    {
        ...
        $fullname = $this->db->select('name')
            ...
            ->get()
            ->row('name');
        $list = $this->M_sam_datadeklarasi->get_datatables();
        $data = array();
        $no = $_POST['start'];
        ...
        foreach ($list as $field) {
            ...
            $action_read = ($read == 'Y') ? '<a href="sam_datadeklarasi/read_form/' . $field->id . '" class="btn btn-info btn-circle btn-sm" title="Read"><i class="fa fa-eye"></i></a>&nbsp;' : '';
            $action_edit = ($edit == 'Y') ? '<a href="sam_datadeklarasi/edit_form/' . $field->id . '    " class="btn btn-warning btn-circle btn-sm" title="Edit"><i class="fa fa-edit"></i></a>&nbsp;' : '';
            $action_delete = ($delete == 'Y') ? '<a onclick="delete_data(' . "'" . $field->id . "'" . ')" class="btn btn-danger btn-circle btn-sm" title="Delete"><i class="fa fa-trash"></i></a>&nbsp;' : '';
            $action_print = ($print == 'Y') ? '<a class="btn btn-success btn-circle btn-sm" target="_blank" href="sam_datadeklarasi/generate_pdf/' . $field->id . '"><i class="fas fa-file-pdf"></i></a>' : '';
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->M_sam_datadeklarasi->count_all(),
            "recordsFiltered" => $this->M_sam_datadeklarasi->count_filtered(),
            "data" => $data,
        );
    ...
    function read_form($id)
    {
        ...
        $data['id'] = $id;
        $data['user'] = $this->M_sam_datadeklarasi->get_by_id($id);
        ...
        $data['title'] = 'backend/sam_datadeklarasi/sam_deklarasi_read';
        $this->load->view('backend/home', $data);
    }
    ...
    function add_form()
    {
        ...
        $data['aksi'] = 'update';
        $data['title'] = 'backend/sam_datadeklarasi/sam_deklarasi_form';
        $this->load->view('backend/home', $data);
    }
    ...
    function edit_form($id)
    {
        ...
        $data['id_pembuat'] = $this->M_sam_datadeklarasi->get_by_id($id)->id_pengaju;
    ...
    {
        $data['master'] = $this->M_sam_datadeklarasi->get_by_id($id);
    ...
    public function generate_kode()
    {
        ...
        $kode = $this->M_sam_datadeklarasi->max_kode($date)->row();
    ...
    public function add()
    {
        ...
        if ($valid) {
            $this->M_sam_datadeklarasi->save($data);
    ...
    public function get_by_kode()
    {
        ...
        $row = $this->db->get_where('sam_deklarasi', array('kode_deklarasi' => $kode))->row_array();
        if (!$row) {
    ...
    public function update_by_kode()
    {
        ...
        $this->db->update('sam_deklarasi', $update);
    ...
    public function update()
    {
        ...
        $this->db->update('sam_deklarasi', $data);
        echo json_encode(array("status" => TRUE));
    }

    function delete($id)
    {
        $this->M_sam_datadeklarasi->delete($id);
        echo json_encode(array("status" => TRUE));
    }
    ...
    public function generate_pdf($id)
    {
        ...
        // Logo
        $pdf->Image(base_url('') . '/assets/backend/img/sml.png', 12, 8, 60, 18);
